...
enum
{
    ...
    PCI_SUBCLASS_SYSTEM_OTHER = 0x80u,
    ...
    PCI_VMD_MMIO_SPAN_HINT = 0x00100000u,
    ...
    PCI64_VMD_NESTED_SCAN_DEVICE_LIMIT = 32u,
    PCI64_VMD_NESTED_SCAN_BUS_LIMIT = 1u,
...
static u32 g_vmd_candidate_count = 0u;
#endif
...
static u32 g_first_vmd_candidate_address = 0xFFFFFFFFu;
static u32 g_first_vmd_candidate_vendor_device = 0u;
...
static u32 g_vmd_nested_plan = 0u;
static u32 g_vmd_nested_enumerated = 0u;
...
static u32 g_vmd_nested_scan_buses = 0u;
static u32 g_vmd_nested_scan_devices = 0u;
static u32 g_vmd_nested_scan_functions = 0u;
static u32 g_vmd_nested_scan_windows = 0u;
static u32 g_vmd_nested_scan_truncated = 0u;
static u32 g_vmd_nested_mmio_base_low = 0u;
static u32 g_vmd_nested_mmio_base_high = 0u;
...
static void pci64_note_function(u32 bus, u32 device, u32 function)
{
    ...
    else if ((class_code == PCI_CLASS_SYSTEM) && (vendor == PCI_INTEL_VENDOR))
    {
        ...
        if (subclass == PCI_SUBCLASS_SYSTEM_OTHER)
        {
            ++g_vmd_candidate_count;
        }
        ...
        if ((subclass == PCI_SUBCLASS_SYSTEM_OTHER)
            && (g_first_vmd_candidate_address == 0xFFFFFFFFu))
        {
            bar0 = pci64_read_config(bus, device, function, 0x10u);
            bar1 = pci64_read_config(bus, device, function, 0x14u);
            g_first_vmd_candidate_address = (bus << 16) | (device << 8) | function;
            g_first_vmd_candidate_vendor_device = vendor_device;
            g_first_vmd_candidate_class = class_register;
            g_first_vmd_candidate_bar0 = bar0;
            g_first_vmd_candidate_bar1 = bar1;
        }
    }
#endif
    ...
    u64 window_base_offset)
{
    ...
    if ((bus < bus_start)
        || ((bus - bus_start) >= PCI64_VMD_NESTED_SCAN_BUS_LIMIT)
    ...
    config_offset =
        ...
        | (((u64)(function & 0x07u)) << 12)
        | (u64)(offset & 0xFCu);
    window_end = window_base_offset + (u64)PCI64_VMD_NESTED_MAP_BYTES;
    if ((window_end <= window_base_offset)
        || (config_offset < window_base_offset)
        || ((config_offset + sizeof(u32)) > window_end))
    {
        return 0xFFFFFFFFu;
    }

    config = (volatile u32 *)(u64)(PCI64_VMD_NESTED_MAP_VIRTUAL_BASE + (config_offset - window_base_offset));
    return *config;
}
...
static void pci64_scan_vmd_nested_config_window(void)
{
    ...
    bus = bus_start;
    g_vmd_nested_scan_buses = PCI64_VMD_NESTED_SCAN_BUS_LIMIT;
    g_vmd_nested_scan_devices = PCI64_VMD_NESTED_SCAN_DEVICE_LIMIT;
    g_vmd_nested_scan_functions = PCI64_VMD_NESTED_SCAN_BUS_LIMIT
        * PCI64_VMD_NESTED_SCAN_DEVICE_LIMIT
        * PCI_MAX_FUNCTION;
    g_vmd_nested_scan_truncated = 1u;

    for (window_index = 0u;
        window_index < (PCI64_VMD_NESTED_SCAN_DEVICE_LIMIT / 2u);
        ++window_index)
    {
        window_base_offset = (u64)window_index * (u64)PCI64_VMD_NESTED_MAP_BYTES;
        window_physical_base = physical_base + window_base_offset;
        if (((window_physical_base <= physical_base) && (window_base_offset != 0ull))
            || (paging64_install_kernel_mmio_mapping(
                    PCI64_VMD_NESTED_MAP_VIRTUAL_BASE,
                    window_physical_base,
                    mapped_pages) == 0u))
        {
            g_vmd_nested_status = PCI64_VMD_NESTED_STATUS_MAP_FAILED;
            return;
        }
        ++g_vmd_nested_scan_windows;

        first_device = window_index * 2u;
        device_limit = first_device + 2u;
        if (device_limit > PCI64_VMD_NESTED_SCAN_DEVICE_LIMIT)
        {
            device_limit = PCI64_VMD_NESTED_SCAN_DEVICE_LIMIT;
        }

        for (device = first_device; device < device_limit; ++device)
        {
...
static void pci64_update_vmd_nested_nvme_mmio_plan(void)
{
    u32 flags = 0u;
    u32 base_low = 0u;
    u32 base_high = 0u;
    ...
    if ((g_vmd_nested_first_bar0 != 0u)
        && (g_vmd_nested_first_bar0 != 0xFFFFFFFFu)
        && ((g_vmd_nested_first_bar0 & PCI_BAR_IO_SPACE) == 0u))
    {
        flags |= PCI64_NVME_MMIO_FLAG_MEMORY_BAR
            | PCI64_NVME_MMIO_FLAG_MAPPING_REQUIRED;

        if ((g_vmd_nested_first_bar0 & PCI_BAR_MEMORY_TYPE_MASK)
            == PCI_BAR_MEMORY_TYPE_64BIT)
        {
            flags |= PCI64_NVME_MMIO_FLAG_64BIT_BAR;
            base_high = g_vmd_nested_first_bar1;
        }

        base_low = g_vmd_nested_first_bar0 & PCI_BAR_MEMORY_BASE_MASK;
        if ((base_low != 0u) || (base_high != 0u))
        {
            flags |= PCI64_NVME_MMIO_FLAG_BASE_NONZERO;
        }
    ...
    token = pci64_mix_token(token, g_vmd_nested_first_bar0);
    token = pci64_mix_token(token, g_vmd_nested_first_bar1);
    token = pci64_mix_token(token, base_low);
    token = pci64_mix_token(token, base_high);
    token = pci64_mix_token(token, PCI_NVME_MMIO_SPAN_HINT);
    token = pci64_mix_token(token, flags);

    g_vmd_nested_mmio_base_low = base_low;
    g_vmd_nested_mmio_base_high = base_high;
    g_vmd_nested_mmio_span_hint =
        ((flags & PCI64_NVME_MMIO_FLAG_PRESENT) != 0u)
            ? PCI_NVME_MMIO_SPAN_HINT
            : 0u;
    g_vmd_nested_mmio_flags = flags;
    g_vmd_nested_mmio_token = token;
}
...
static void pci64_update_vmd_nested_plan(void)
{
    ...
    u32 register_token = 2166136261u;
    ...
    u32 usable_mmio =
        ...
        && ((g_first_vmd_candidate_mmio_flags & PCI64_VMD_MMIO_FLAG_BASE_NONZERO) != 0u)
    ...
    g_vmd_nested_scan_buses = 0u;
    g_vmd_nested_scan_devices = 0u;
    g_vmd_nested_scan_functions = 0u;
    g_vmd_nested_scan_windows = 0u;
    g_vmd_nested_scan_truncated = 0u;
    g_vmd_nested_mmio_base_low = 0u;
    g_vmd_nested_mmio_base_high = 0u;
    g_vmd_nested_mmio_span_hint = 0u;
    g_vmd_nested_mmio_flags = 0u;
    ...
    else if (g_vmd_nested_bind_ready == 0u)
    {
        g_vmd_nested_register_status = PCI64_VMD_NESTED_REGISTER_STATUS_WAITING_BIND_READY;
    }
    else
    {
        g_vmd_nested_register_candidate = 1u;
        g_vmd_nested_register_status = PCI64_VMD_NESTED_REGISTER_STATUS_DEFERRED_NO_DRIVER;
    }

    register_token = pci64_mix_token(register_token, g_vmd_nested_register_candidate);
    register_token = pci64_mix_token(register_token, g_vmd_nested_register_status);
    register_token = pci64_mix_token(register_token, g_vmd_nested_bind_ready);
    register_token = pci64_mix_token(register_token, g_vmd_nested_bind_token);
    register_token = pci64_mix_token(register_token, g_vmd_nested_first_address);
    register_token = pci64_mix_token(register_token, g_vmd_nested_mmio_base_low);
    register_token = pci64_mix_token(register_token, g_vmd_nested_mmio_base_high);
    ...
    token *= 16777619u;
    token ^= g_vmd_nested_enumerated;
    token *= 16777619u;
    token ^= g_vmd_nested_nvme_count;
    token *= 16777619u;
    ...
    token *= 16777619u;
    token ^= g_vmd_nested_scan_buses;
    token *= 16777619u;
    token ^= g_vmd_nested_scan_devices;
    token *= 16777619u;
    token ^= g_vmd_nested_scan_functions;
    token *= 16777619u;
    token ^= g_vmd_nested_scan_windows;
    token *= 16777619u;
    token ^= g_vmd_nested_scan_truncated;
    token *= 16777619u;
    token ^= g_vmd_nested_mmio_base_low;
    token *= 16777619u;
    token ^= g_vmd_nested_mmio_base_high;
    token *= 16777619u;
    token ^= g_vmd_nested_mmio_span_hint;
    token *= 16777619u;
    ...
    token *= 16777619u;
    token ^= g_vmd_nested_register_candidate;
    token *= 16777619u;
    token ^= g_vmd_nested_register_status;
    token *= 16777619u;
    token ^= g_vmd_nested_register_token;
    token *= 16777619u;
...
void pci64_init(const struct boot_info *boot_info)
{
    ...
    g_first_vmd_candidate_address = 0xFFFFFFFFu;
    g_first_vmd_candidate_vendor_device = 0u;
...
{
    return pci64_authorized_value(hardware_capability_handle, owner_id, g_vmd_candidate_count);
}
#endif
...
{
    return pci64_authorized_value(hardware_capability_handle, owner_id, g_first_vmd_candidate_address);
}

u32 pci64_first_vmd_candidate_vendor_device(u32 hardware_capability_handle, u32 owner_id)
{
    return pci64_authorized_value(hardware_capability_handle, owner_id, g_first_vmd_candidate_vendor_device);
}

u32 pci64_first_vmd_candidate_class(u32 hardware_capability_handle, u32 owner_id)
{
    return pci64_authorized_value(hardware_capability_handle, owner_id, g_first_vmd_candidate_class);
}

u32 pci64_first_vmd_candidate_bar0(u32 hardware_capability_handle, u32 owner_id)
{
    return pci64_authorized_value(hardware_capability_handle, owner_id, g_first_vmd_candidate_bar0);
}
...
{
    return pci64_authorized_value(hardware_capability_handle, owner_id, g_vmd_nested_plan);
}

u32 pci64_vmd_nested_enumerated(u32 hardware_capability_handle, u32 owner_id)
{
    return pci64_authorized_value(hardware_capability_handle, owner_id, g_vmd_nested_enumerated);
}

u32 pci64_vmd_nested_nvme_count(u32 hardware_capability_handle, u32 owner_id)
{
    return pci64_authorized_value(hardware_capability_handle, owner_id, g_vmd_nested_nvme_count);
}

u32 pci64_vmd_nested_status(u32 hardware_capability_handle, u32 owner_id)
{
    return pci64_authorized_value(hardware_capability_handle, owner_id, g_vmd_nested_status);
}
...
{
    return pci64_authorized_value(hardware_capability_handle, owner_id, g_vmd_nested_scan_buses);
}

u32 pci64_vmd_nested_scan_devices(u32 hardware_capability_handle, u32 owner_id)
{
    return pci64_authorized_value(hardware_capability_handle, owner_id, g_vmd_nested_scan_devices);
}

u32 pci64_vmd_nested_scan_functions(u32 hardware_capability_handle, u32 owner_id)
{
    return pci64_authorized_value(hardware_capability_handle, owner_id, g_vmd_nested_scan_functions);
}

u32 pci64_vmd_nested_scan_windows(u32 hardware_capability_handle, u32 owner_id)
{
    return pci64_authorized_value(hardware_capability_handle, owner_id, g_vmd_nested_scan_windows);
}

u32 pci64_vmd_nested_scan_truncated(u32 hardware_capability_handle, u32 owner_id)
{
    return pci64_authorized_value(hardware_capability_handle, owner_id, g_vmd_nested_scan_truncated);
}

u32 pci64_vmd_nested_mmio_base_low(u32 hardware_capability_handle, u32 owner_id)
{
    return pci64_authorized_value(hardware_capability_handle, owner_id, g_vmd_nested_mmio_base_low);
}

u32 pci64_vmd_nested_mmio_base_high(u32 hardware_capability_handle, u32 owner_id)
{
    return pci64_authorized_value(hardware_capability_handle, owner_id, g_vmd_nested_mmio_base_high);
}

u32 pci64_vmd_nested_mmio_span_hint(u32 hardware_capability_handle, u32 owner_id)
{
    return pci64_authorized_value(hardware_capability_handle, owner_id, g_vmd_nested_mmio_span_hint);
}

u32 pci64_vmd_nested_mmio_flags(u32 hardware_capability_handle, u32 owner_id)
{
    return pci64_authorized_value(hardware_capability_handle, owner_id, g_vmd_nested_mmio_flags);
}
...
{
    return pci64_authorized_value(hardware_capability_handle, owner_id, g_vmd_nested_register_candidate);
}

u32 pci64_vmd_nested_register_status(u32 hardware_capability_handle, u32 owner_id)
{
    return pci64_authorized_value(hardware_capability_handle, owner_id, g_vmd_nested_register_status);
}

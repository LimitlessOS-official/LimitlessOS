...
#else
    return shell64_write_text(console_capability_handle, owner_id, "Product services: BIOS fallback shows service and session status; installer UX unavailable\n");
#endif
}
...
static u32 shell64_print_package_status(u32 console_capability_handle, u32 owner_id)
{
    ...
    (void)shell64_write_text(console_capability_handle, owner_id, "local association: active and offline-capable\n");
    ...
    (void)shell64_write_text(console_capability_handle, owner_id, "cloud provider descriptor: signed local provider verified\n");
...
static u32 shell64_print_hardware_validation_status(u32 console_capability_handle, u32 owner_id)
{
    ...
    (void)shell64_write_hwval_text_line(console_capability_handle, owner_id, "machine model: not reported by Product\n");
    ...
    u32 owner_id)
{
    ...
    (void)shell64_write_text(console_capability_handle, owner_id, "Identity, account, vault, and transport status: Settings; local only; no secret storage\n");
    ...
    u32 owner_id)
{
    ...
    if (shell64_token_equals(command_start, command_length, "help"))
    {
        ...
        (void)shell64_write_text(console_capability_handle, owner_id, "Product cloud storage: Settings and File Manager show broker policy; sync unavailable; transfers denied\n");

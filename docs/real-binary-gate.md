...
Current status: the first static Linux x86_64 ELF execution gate, the M22 per-process page table foundation gate, and the M23 bounded fork/wait gate are crossed on the UEFI Product path. The verified commands are `linux /APPS/BUSYBOX echo limitless-real-binary`, `linux /APPS/BUSYBOX cat /proc/meminfo`, `linux /APPS/BUSYBOX cat /nvme/apps/data/file.txt`, `linux /APPS/BUSYBOX ls /nvme/apps`, `linux /APPS/BUSYBOX ls -l /proc/self/exe`, `linux /APPS/BUSYBOX ls -l /proc/self/fd`, `linux /APPS/BUSYBOX ls -l /proc/self`, `linux /APPS/BUSYBOX sh`, `linux /APPS/BUSYBOX sh -c 'echo m22-cr3-isolation'`, and `linux /APPS/BUSYBOX sh -c 'ls /nvme/apps; ls /nvme/apps/data'`, using externally built BusyBox 1.35.0 static musl ET_EXEC binaries staged on the NVMe FAT image at `/APPS/BUSYBOX`.
...
- Process creation and reaping: bounded `fork(57)` full-copies the current Linux persona process root, lower-half VMA/address-space mappings, FD table, persona state, VFS state, and audit state for the observed BusyBox ash external-command path; the child returns 0, the parent receives the child pid, blocking `wait4(61)` reaps the child exit status, and both PML4 slots are returned to the static pool. Broad `clone` flags, `vfork`, COW, threading, and signal delivery remain outside this gate.
- Exit: `exit_group`.
...
M24 should be `CR3 repair and child stdin inheritance trace`: first identify why the M23 run reports `syscall-root-repair 1`, then exercise child-side stdin inheritance because the accepted fork run reports `read 0 read-bytes 0`. The scope should remain trace-driven and narrow: improve or explain the CR3 repair path, prove inherited stdin through a real forked BusyBox child, and avoid dynamic linking, glibc, vfork, real threading, signal delivery, sockets, broad clone flag compatibility, or broad ioctl/device control.

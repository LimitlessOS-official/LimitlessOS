...
Proposed M48 scope: `absolute localbin repeated-slash path proof`. M48 should trace `linux /APPS/BUSYBOX sh -c '/usr//local/bin/sbenv USER=operator /usr/local//bin/sbenv|sbcat'` and make the minimum change needed, if any, so repeated slash segments canonicalize into the bounded `/usr/local/bin` alias table for both shell-launched children and non-shell handoffs.

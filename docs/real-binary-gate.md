...
Proposed M50 scope: `absolute over-root dotdot clamp proof`. M50 should trace `linux /APPS/BUSYBOX sh -c '/../../usr/local/bin/sbenv USER=operator /../../../usr/local/bin/sbenv|sbcat'` and make the minimum change needed, if any, so over-root `..` traversal clamps at `/` and canonicalizes into the `/usr/local/bin` alias table for both shell-launched children and non-shell handoffs.

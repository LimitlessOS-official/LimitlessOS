...
Proposed M44 scope: `non-shell current-directory execvp proof`. M44 should trace `linux /APPS/BUSYBOX sh -c 'cd /usr/local/bin; ./sbenv USER=operator ./sbenv | ./sbcat'` and make the minimum change needed, if any, so a third-party process can perform an explicit relative `./` handoff from `/usr/local/bin` after BusyBox ash launches it the same way.

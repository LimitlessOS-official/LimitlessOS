...
Proposed M40 scope: `absolute localbin file-read proof`. M40 should trace `linux /APPS/BUSYBOX sh -c '/usr/local/bin/sbcat /nvme/apps/data/file.txt'` and make the minimum change needed, if any, so an absolute `/usr/local/bin` third-party utility can read a real NVMe file directly through its own child `execve`, not just consume pipeline input.

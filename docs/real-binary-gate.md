...
Proposed M46 scope: `absolute localbin executable dotdot path proof`. M46 should trace `linux /APPS/BUSYBOX sh -c '/usr/local/bin/../bin/sbenv USER=operator /usr/local/bin/../bin/sbenv | /usr/local/bin/../bin/sbcat'` and make the minimum change needed, if any, so absolute executable `..` segments canonicalize into the bounded `/usr/local/bin` alias table for both shell-launched children and non-shell handoffs.

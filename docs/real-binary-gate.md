...
Proposed M29 scope: `broader third-party utility read path`. M29 should build and stage a second upstream sbase utility such as `cat` or `wc`, prove direct file input from `/nvme/apps/data/file.txt`, and prove forked pipeline composition against another executable without relying on BusyBox applets for the data consumer. It should remain narrower than dynamic linking, glibc, vfork, real threading, signal delivery, sockets, broad clone flag compatibility, broad ioctl/device control, or arbitrary distro filesystem population.

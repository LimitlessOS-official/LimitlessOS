...
Proposed M55 scope: `non-shell execvp missing executable denial proof`. M55 should exercise a missing command requested by a third-party `sbenv` process through `execvp` and prove the same PATH denial semantics hold outside BusyBox ash itself, with no unintended NVMe backend reads, pipe leaks, PML4 leaks, or CR3 repair.

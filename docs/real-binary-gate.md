...
Proposed M43 scope: `localbin current-directory path proof`. M43 should trace `linux /APPS/BUSYBOX sh -c 'cd /usr/local/bin; ./sbcat /nvme/apps/data/file.txt | ./sbcat'` and make the minimum change needed, if any, so the bounded `/usr/local/bin` aliases can also be executed through current-directory relative `./` paths, not only PATH search and absolute paths.

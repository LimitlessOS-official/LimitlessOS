...
Proposed M37 scope: `all-third-party PATH pipeline proof`. M37 should trace first and then make the minimum change needed, if any, so BusyBox ash and the non-shell `sbenv` child can both resolve third-party utilities by PATH name in one command, for example `linux /APPS/BUSYBOX sh -c 'sbenv USER=operator sbenv | sbcat'`. This should prove a fully PATH-resolved third-party pipeline without broadening into dynamic linking, glibc, vfork, real threading, signal delivery, sockets, broad clone flag compatibility, broad ioctl/device control, or arbitrary distro filesystem population.

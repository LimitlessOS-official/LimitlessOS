...
Proposed M45 scope: `localbin executable dotdot path proof`. M45 should trace `linux /APPS/BUSYBOX sh -c 'cd /usr/local/bin; ../bin/sbenv USER=operator ../bin/sbenv | ../bin/sbcat'` and make the minimum change needed, if any, so executable `..` segments canonicalize into the bounded `/usr/local/bin` alias table for both shell-launched children and non-shell handoffs.

...
Proposed M52 scope: `absolute localbin trailing-slash directory/open proof`. M52 should exercise trailing slash behavior on directory-oriented operations, such as `/usr/local/bin/`, and confirm directory operations still work while executable file paths with terminal slashes remain denied.

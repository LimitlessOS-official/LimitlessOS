...
Proposed M57 scope: `non-shell execvp directory-target denial proof`. M57 should exercise a directory path requested by a third-party `sbenv` process through `execvp`, such as `/usr/local/bin/`, and prove executable directory targets are denied cleanly without unintended binary reads, pipe leaks, PML4 leaks, or CR3 repair.

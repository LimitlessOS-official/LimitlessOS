...
Proposed M41 scope: `localbin cwd-relative file-read proof`. M41 should trace `linux /APPS/BUSYBOX sh -c 'cd /nvme/apps; /usr/local/bin/sbcat data/file.txt | /usr/local/bin/sbcat'` and make the minimum change needed, if any, so absolute-localbin third-party utilities inherit BusyBox ash cwd and can read cwd-relative NVMe paths through forked child `execve`.

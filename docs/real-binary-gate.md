...
Proposed M54 scope: `PATH localbin missing executable denial proof`. M54 should exercise a missing command resolved by BusyBox ash through the default `PATH` and prove `/usr/local/bin`, `/bin`, and `/usr/bin` lookup failures report `not found` without touching unintended NVMe backend reads, leaking pipe/PML4 state, or regressing valid alias lookup.

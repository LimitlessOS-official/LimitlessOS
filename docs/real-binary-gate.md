...
Proposed M38 scope: `expanded localbin directory proof`. M38 should trace first and then make the minimum change needed, if any, so `linux /APPS/BUSYBOX sh -c 'ls /usr/local/bin'` visibly enumerates `sbcat`, `sbecho`, and `sbenv` with no stat denials. This should prove the bounded third-party PATH namespace is inspectable as well as executable without broadening into arbitrary distro filesystem population.

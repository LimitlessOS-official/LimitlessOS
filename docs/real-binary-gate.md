...
Proposed M61 scope: `non-shell execvp parent-rebased directory executable denial proof`. M61 should exercise `/usr/local/bin/../bin` through third-party `sbenv` `execvp`, proving `..` canonicalization back into `/usr/local/bin` still rejects executable use before any backend binary read.

...
## M171 MSI Boot-Latency Timing Telemetry

M171 responds to the physical MSI report that the laptop takes roughly 3-5 minutes to reach the Product desktop while VirtualBox reaches the desktop much faster. The UEFI Product boot path now records bounded tick durations for xHCI init, boot diagnostics, NVMe probe, display init, I2C HID init, timer wait, keyboard wait, mouse wait, login, lock probe, window-manager probe, desktop probe, virtio-net init, and PIT-to-shell total. The kernel prints `[x64] boot-time ... ticks ...` during boot and exposes the same data through `hwval` as `boot ticks ...` lines.

Accepted verification: Product build with staged dynamic artifacts and M1 production-slice gate passed; BIOS reserve remained `101` sectors; UEFI reserve is `716,352` bytes; the current MSI handoff bundle self-verified with ISO SHA-256 `619ffac1d528b499438631c7a17dcab07d94b1b1c786cc32ef1ba8f9a8131d41` and UEFI image SHA-256 `1940c39fdca70fc203f49ba68a38145b7c2e9c333e0f9e41c58c33669280ba34`; and `tools\verify-hardware-display-input-fixtures.ps1` passed `39/39`. M171 does not claim a boot-speed improvement; it makes the next physical run identify which real hardware stage owns the delay.

## M170 MSI xHCI Mouse Descriptor Fallback/Evidence

M170 responds to the physical MSI desktop photos where the GUI reached the shell with a visible but non-moving cursor, the USB mouse dongle and built-in touchpad did not drive pointer motion, and `hwval` showed xHCI present with firmware handoff complete but no HID device bound (`USB2 12 ERR 27`). The UEFI Product xHCI probe now has a conservative broad mouse fallback for non-hub, non-boot-keyboard interrupt-IN interfaces with mouse-sized packets, covering vendor-specific/composite USB mouse receivers without fabricating movement. Real pointer movement still requires real xHCI reports to reach the brokered input queue.

`hwval` now prints descriptor evidence for the next laptop run: xHCI last skip port/code, last device class/subclass/protocol, last configuration length, last interface class/subclass/protocol, last endpoint max packet, and broad mouse probe count. The host-side display/input analyzer preserves these fields and adds `xhci-mouse-descriptor-unmatched` for captures that still stop at xHCI error `27`.

Accepted verification: Product build with staged dynamic artifacts and M1 production-slice gate passed; BIOS reserve remained `101` sectors; UEFI reserve is `716,896` bytes; the current MSI handoff bundle self-verified with ISO SHA-256 `d3942bb40ed3f571f3f7d4f1a1f557343e4365df686d13f3b9e7c3c51454736b` and UEFI image SHA-256 `dc86c45602a0e524693693ec70944b203fa8b4bf4decf96a0b61a6a1e27393c3`; `tools\verify-msi-hardware-handoff.ps1 -RequireStagedDynamicArtifacts -RequireGuiInteractionTelemetry` passed; `tools\start-msi-hardware-capture.ps1` generated a fresh session; and `tools\verify-hardware-display-input-fixtures.ps1` passed `39/39`. A UEFI Product QEMU verifier attempt reached the shell and logged xHCI input live, but timed out before a clean verifier result, so M170 does not claim a QEMU verifier pass or a physical MSI mouse pass.

## M169 MSI PIC MASK Boot-Stage Diagnostics

M169 adds UEFI-only diagnostic markers for the physical MSI boot freeze observed after the existing `PIC MASK` line. Immediately after `PIC MASK`, the kernel now prints APIC/PIC/IRQ route state (`apic`, `pic-disabled`, `timer-gsi`, `keyboard-gsi`, and `mouse-gsi`) and emits narrower stage markers around the next boot steps: `PIT INIT`, `PIT OK`, `SYSCALL INIT`, `SYSCALL OK`, `INT PROBES`, and `INT PROBES OK`. The markers are under `LIMITLESS_X64_UEFI_KERNEL` so BIOS code size and behavior stay protected.

Accepted verification: Product x86_64 build with staged `/APPS/DYNLDLIMIT` and `/APPS/LDLIMIT`, M1 production-slice gate pass, `tools\prepare-hardware-storage-evidence.ps1 -EvidenceDir .\dist\m133-msi-hardware-handoff-current -SkipBuild -SkipQemuGate`, `tools\verify-msi-hardware-handoff.ps1 -EvidenceDir .\dist\m133-msi-hardware-handoff-current -RequireStagedDynamicArtifacts -RequireGuiInteractionTelemetry`, and `tools\start-msi-hardware-capture.ps1`. The refreshed current handoff bundle records ISO SHA-256 `69e95ab98684eb84112497de603ed9160a4563a0db20b54bddbf42d2f380eec6`, UEFI image SHA-256 `b2cdf962eed8b831d71f590ed646dc4794959211cbc88d5b74ac47db063d13e0`, `/APPS/DYNLDLIMIT` SHA-256 `9f6eb9c05b3065d39bc59d24defe9361267b34cefd4de78f568ddb00497238fa`, and `/APPS/LDLIMIT` SHA-256 `6f713105878c30d817b7add4a7ed5d4ee8e01fb6eab2c80ba10acee059c72238`. Final reserves are BIOS `101` sectors and UEFI `721,280` bytes. M169 does not claim a physical MSI pass or a fixed cursor; it narrows the next real hardware failure from broad `PIC MASK` to a precise boot-stage marker.

## M167 MSI Hardware Capture Session Helper

M167 adds `tools\start-msi-hardware-capture.ps1`, a host-side command for starting the next physical MSI run from the current verified handoff bundle. The helper verifies `dist\m133-msi-hardware-handoff-current` with the strict staged-dynamic and GUI telemetry requirements, writes `dist\msi-hardware-capture-session\msi-hardware-capture-session.txt` with the exact ISO path, SHA-256 values, reserves, laptop commands, required telemetry lines, capture path, and report command, and writes a blank transcript template. It does not create or synthesize hardware evidence.

Accepted verification: running `tools\start-msi-hardware-capture.ps1` against the M166 bundle reverified `source2-required: 2`, `nvme-controller-snapshot-required: 1`, BIOS reserve `101` sectors, and UEFI reserve `721,408` bytes. M167 does not claim a physical MSI pass, a working VMD-backed NVMe controller, successful internal SSD FAT access, controller writes, or safe internal-disk writes.

## M168 MSI Hardware Capture Finish Helper

M168 adds `tools\finish-msi-hardware-capture.ps1`, the paired post-capture command for the M167 session helper. It validates that the capture file exists, is non-empty, and is not still the blank transcript template, then runs the strict M155 report path with staged dynamic artifacts and GUI interaction telemetry required. It writes `msi-hardware-capture-next-target.txt` and prints `target-kind`, `target-stage`, `roadmap-target`, `next-target`, and transcript-presence booleans for the expected `hwval`, `linux /APPS/DYNLDLIMIT`, `drs-nvme-triage`, and `drs-realbin` evidence.

Accepted verification: `m134-storage-target-fixtures: 17/17`, a wrapper run against the generated `storage-ready` synthetic fixture with report exit `0`, and a blank-template rejection test. M168 does not claim a physical MSI pass, a working VMD-backed NVMe controller, successful internal SSD FAT access, controller writes, or safe internal-disk writes.

## M166 Current MSI Handoff Bundle M165 Refresh

M166 refreshes the ignored physical handoff bundle at `dist\m133-msi-hardware-handoff-current` from commit `c38296b1` so the USB handoff package matches the M165 stale-capture verifier/report behavior. The refreshed bundle records ISO SHA-256 `b3ce83b485b4bf3de5bad8e9622c7bfc518c071202359894c092a19adf365bc3`, UEFI image SHA-256 `7d87db946fac05f6e436c74046b23cf9f7a4985f860bc46630277155f57109b7`, `/APPS/DYNLDLIMIT` SHA-256 `9f6eb9c05b3065d39bc59d24defe9361267b34cefd4de78f568ddb00497238fa`, and `/APPS/LDLIMIT` SHA-256 `6f713105878c30d817b7add4a7ed5d4ee8e01fb6eab2c80ba10acee059c72238`.

The staged Product build copied `/APPS/DYNLDLIMIT` and `/APPS/LDLIMIT` into the UEFI boot image, passed the M1 production-slice gate, and produced final reserves of BIOS `101` sectors and UEFI `721,408` bytes. The handoff bundle was regenerated with `-SkipBuild -SkipQemuGate`; the bundle self-verifier reports `source2-required: 2`, `nvme-controller-snapshot-required: 1`, and the nine required controller fields, while `qemu-storage-stage-gate-skipped.txt` records that no stale QEMU storage proof is present. M166 does not claim a physical MSI pass, a working VMD-backed NVMe controller, successful internal SSD FAT access, controller writes, or safe internal-disk writes.

## M165 MSI Capture NVMe Snapshot Stale-Transcript Routing

M165 is accepted as host-side capture/report hardening for the current MSI storage handoff. When a handoff manifest requires the M163 NVMe controller snapshot and a real `-CapturePath` transcript is supplied, `tools\verify-hardware-storage-evidence.ps1` now checks the raw `drs-nvme-triage` line for all nine required fields: `nvme-probe-error`, `nvme-regs`, `nvme-cap-low`, `nvme-cap-high`, `nvme-vs`, `nvme-cc`, `nvme-csts`, `nvme-dstrd-bytes`, and `nvme-doorbell-page`.

If any required field is missing, the capture is routed to `nvme-controller-snapshot-missing`, the missing field list is recorded, the next target instructs the tester to rerun `hwval` on an M163-or-newer Product image, and promoted controller snapshot fields remain blank in JSON/text/Markdown output instead of being rendered as misleading zero values. The capture-side next target is preserved through the combined MSI analyzer, the M134 classifier, the handoff verifier, and the final MSI capture report.

Accepted verification: `m134-storage-target-fixtures: 17/17`, `msi-hardware-capture-report-fixtures: 4/4`, `msi-hardware-handoff-fixtures: 17/17`, and Product build plus M1 production-slice gate. Final reserves remain BIOS `101` sectors and UEFI `721,408` bytes. M165 does not claim a physical MSI pass, a working VMD-backed NVMe controller, successful internal SSD FAT access, controller writes, or safe internal-disk writes.

## M164 Current MSI Handoff Bundle M163 Refresh

M164 refreshes the ignored physical handoff bundle at `dist\m133-msi-hardware-handoff-current` so the actual USB handoff package carries the M163 NVMe controller snapshot contract. The regenerated manifest records commit `ef31c577`, ISO SHA-256 `affbf09c3e7b460c090ea80c0473e6eed6623f76772db74f65bbcedecb8a8ec4`, UEFI image SHA-256 `ce92827618721dcdb326f2aebedc441e304ae9bfccfcf2d7c3f9bb955a47601a`, `/APPS/DYNLDLIMIT` SHA-256 `9f6eb9c05b3065d39bc59d24defe9361267b34cefd4de78f568ddb00497238fa`, and `/APPS/LDLIMIT` SHA-256 `6f713105878c30d817b7add4a7ed5d4ee8e01fb6eab2c80ba10acee059c72238`.

The embedded handoff verifier reports `source2-required: 2`, `nvme-controller-snapshot-required: 1`, and all nine required controller fields. `tools\prepare-hardware-storage-evidence.ps1` now removes stale `qemu-storage-stage-gate.txt` files when `-SkipQemuGate` is used and writes `qemu-storage-stage-gate-skipped.txt`, so an in-place refresh cannot leave old QEMU proof inside a current bundle. Accepted verification: staged Product build plus M1 production-slice gate during the refresh attempt, followed by `tools\prepare-hardware-storage-evidence.ps1 -EvidenceDir .\dist\m133-msi-hardware-handoff-current -SkipBuild -SkipQemuGate` and self-verification. Final reserves remain BIOS `101` sectors and UEFI `721,408` bytes. M164 does not claim a physical MSI pass, a working VMD-backed NVMe controller, successful internal SSD FAT access, controller writes, or safe internal-disk writes.

## M163 MSI Handoff NVMe Controller Snapshot Contract

M163 is accepted as host-side contract hardening for the M161/M162 NVMe controller snapshot. Generated MSI handoff bundles now declare `required_nvme_controller_snapshot = 1` and list the required fields: `nvme-probe-error`, `nvme-regs`, `nvme-cap-low`, `nvme-cap-high`, `nvme-vs`, `nvme-cc`, `nvme-csts`, `nvme-dstrd-bytes`, and `nvme-doorbell-page`.

`tools\verify-msi-hardware-handoff.ps1` now rejects stale bundles whose manifest or runbook omit the snapshot requirement, and the runbook instructs the hardware tester to confirm the same values under `NVMe Controller Snapshot` in the generated report. Fixture bundle generators were updated to the same contract so M134 target classification and MSI handoff verification cannot regress to a pre-M161/M162 capture shape. Accepted verification: `m134-storage-target-fixtures: 16/16`, `msi-hardware-handoff-fixtures: 16/16`, `msi-hardware-capture-report-fixtures: 3/3`, and Product build plus M1 production-slice gate. Final reserves remain BIOS `101` sectors and UEFI `721,408` bytes. M163 does not claim a physical MSI pass, a working VMD-backed NVMe controller, successful internal SSD FAT access, controller writes, or safe internal-disk writes.

## M162 MSI NVMe Controller Snapshot Report Surfacing

M162 is accepted as host-side report surfacing for the M161 raw NVMe controller snapshot. The hardware storage evidence verifier now promotes analyzer `key_fields` into a stable `capture_nvme_controller` object, formatting CAP/VS/CC/CSTS as padded hex strings while preserving counters as decimal text. The combined MSI analyzer, M134 target classifier, and one-command MSI hardware capture report all carry the object through JSON, text, and Markdown outputs.

Fixture coverage asserts the snapshot survives the full chain from analyzer output to manager-facing report: `probe_error 0`, `regs 1`, `cap_low 0x00003FFF`, `cap_high 0x00000030`, `vs 0x00010400`, `cc 0x00460001`, and `csts 0x00000001`. Accepted verification: `hardware-storage-analysis-fixtures: 67/67`, `m134-storage-target-fixtures: 16/16`, `msi-hardware-capture-report-fixtures: 3/3`, and Product build plus M1 production-slice gate. Final reserves remain BIOS `101` sectors and UEFI `721,408` bytes. M162 does not claim a physical MSI pass, a working VMD-backed NVMe controller, successful internal SSD FAT access, controller writes, or safe internal-disk writes.

## M161 NVMe Controller Register Snapshot Diagnostics

M161 is accepted as controller-level storage diagnostics for both direct PCI NVMe and the M159 VMD-bound candidate path. The UEFI Product NVMe probe now records a read-only MMIO register snapshot after the controller register window is mapped and refreshes it at the probe success/failure boundary. The snapshot includes `nvme-probe-error`, `nvme-regs`, `nvme-cap-low`, `nvme-cap-high`, `nvme-vs`, `nvme-cc`, `nvme-csts`, `nvme-dstrd-bytes`, and `nvme-doorbell-page` through the user-facing `drs-nvme-triage` line and the lower-level `drs-nvme-probe` proof.

The hardware storage analyzer now treats those fields as key evidence for `nvme-controller-discovery`, `nvme-controller-ready`, and `nvme-identify` stages. A future MSI transcript that reaches a mapped controller but fails readiness or Identify can show raw CAP/VS/CC/CSTS state and doorbell stride/page details instead of collapsing to only `nvme-ready 0`. Accepted verification: Product build plus M1 production-slice gate, `hardware-storage-analysis-fixtures: 67/67`, and `m134-storage-target-fixtures: 16/16`. Final reserves: BIOS `101` sectors and UEFI `721,408` bytes. M161 does not claim a physical MSI pass, a working VMD-backed NVMe controller, successful internal SSD FAT access, controller writes, or safe internal-disk writes.

## M160 VMD/NVMe Bound-Path Fixture/Report Hardening

M160 is accepted as host-side hardening for the M159 scoped VMD/NVMe bind path. The storage capture parser now detects `vmd-nvme-bind-state 2` before applying the older deferred-source validation, so a bound candidate promoted to source `3` is no longer misclassified as a stale deferred source. New fixtures prove the bound path routes source-promotion mismatch to `pci-vmd-nested-nvme-bind` and routes successful bound handoffs onward to `nvme-controller-discovery`, `nvme-controller-ready`, and `nvme-identify` when those later stages fail.

The M134 classifier/report path now preserves VMD driver bind state, token, and count in text and Markdown output. Accepted verification: Product build plus M1 production-slice gate, `hardware-storage-analysis-fixtures: 67/67`, and `m134-storage-target-fixtures: 16/16`. Final reserves: BIOS `101` sectors and UEFI `721,696` bytes. M160 does not claim a physical MSI pass, a working VMD-backed NVMe controller, successful internal SSD FAT access, or safe internal-disk writes.

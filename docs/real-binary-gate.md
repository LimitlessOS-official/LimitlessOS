...
Proposed M35 scope: `third-party exec handoff proof`. M35 should trace first, then make the minimum change needed, if any, so the newly staged sbase `env` utility can `execvp` another real third-party utility, for example `linux /APPS/BUSYBOX sh -c '/nvme/apps/sbenv USER=operator /nvme/apps/sbenv | /nvme/apps/sbcat'`. This would prove a non-shell third-party process can replace itself with another real ET_EXEC while carrying an explicit environment, without broadening into dynamic linking, glibc, vfork, real threading, signal delivery, sockets, broad clone flag compatibility, broad ioctl/device control, or arbitrary distro filesystem population.

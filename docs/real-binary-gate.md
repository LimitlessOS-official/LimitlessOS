...
Proposed M53 scope: `absolute localbin missing executable denial proof`. M53 should exercise a missing `/usr/local/bin/...` command and prove the bounded localbin alias table denies it without accidentally touching NVMe backend reads, leaking pipe/PML4 state, or regressing valid alias lookup.

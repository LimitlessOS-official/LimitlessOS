...
Proposed M47 scope: `absolute localbin mixed-dot path proof`. M47 should trace `linux /APPS/BUSYBOX sh -c '/usr/local/./bin/../bin/sbenv USER=operator /usr/local/./bin/../bin/sbenv | /usr/local/./bin/../bin/sbcat'` and make the minimum change needed, if any, so mixed absolute executable `.` and `..` segments canonicalize into the bounded `/usr/local/bin` alias table for both shell-launched children and non-shell handoffs.

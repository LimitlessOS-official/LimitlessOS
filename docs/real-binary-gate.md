...
Proposed M49 scope: `absolute localbin root-clamped dotdot path proof`. M49 should trace `linux /APPS/BUSYBOX sh -c '/usr/local/bin/../../local/bin/sbenv USER=operator /usr/local/bin/../../local/bin/sbenv|sbcat'` and make the minimum change needed, if any, so bounded upward `..` traversal canonicalizes back into the `/usr/local/bin` alias table for both shell-launched children and non-shell handoffs.

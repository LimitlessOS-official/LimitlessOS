...
Proposed M60 scope: `non-shell execvp parent-directory executable denial proof`. M60 should exercise `/usr/local/bin/..` through third-party `sbenv` `execvp`, proving `..` canonicalization to a parent directory still rejects executable use before any backend binary read.

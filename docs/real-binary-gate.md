...
Proposed M102 scope: expose and exercise the direct generated libc wrapper for existing `pipe(22)` from a dynamic ET_EXEC artifact, prove read/write/close over a kernel pipe fd pair without filesystem mutation, and only then decide whether to broaden into dynamic fork-plus-pipe composition.

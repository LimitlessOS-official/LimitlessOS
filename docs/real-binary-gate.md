...
Proposed M39 scope: `absolute localbin exec proof`. M39 should trace first and then make the minimum change needed, if any, so `linux /APPS/BUSYBOX sh -c '/usr/local/bin/sbenv USER=operator /usr/local/bin/sbenv | /usr/local/bin/sbcat'` executes entirely through absolute `/usr/local/bin` aliases. This should prove the bounded third-party namespace works for direct absolute Linux VFS execution as well as PATH search and directory inspection.

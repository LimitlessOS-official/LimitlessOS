...
Proposed M59 scope: `non-shell execvp dot-directory executable denial proof`. M59 should exercise `/usr/local/bin/.` through third-party `sbenv` `execvp`, proving dot-segment canonicalization still rejects a directory target before any executable backend read.

...
M7.1 proves signed archive/payload admission, signed update-index anti-rollback, and separate deterministic negative fixture coverage. M8 adds read-only trust visibility in Settings and the `pkginfo` shell builtin: users can inspect package mode, signer key id/fingerprint, signed package count, signature/hash status, capability request/admission/denial status, local signed update-index status, rollback/replay status, and unavailable install/update actions. M9 does not change package admission; it keeps package install/apply, app-store behavior, auto-install, live public update fetching, persistent downloaded package installation, and expiry enforcement without trusted time unavailable. Signature proves origin and integrity; capability policy still controls what authority a package can receive.

...
M73 is `bounded dynamic app and interpreter mapping`. The UEFI Product Linux persona launcher still denies dynamic ELF before relocation or control transfer, but a supported `PT_INTERP` request now allocates a bounded Linux persona, maps the dynamic app `PT_LOAD` segments and the staged interpreter candidate `PT_LOAD` segments into that private process root, reports the mapped-page telemetry, and then cleans up the VMA/process-root state. Dynamic linking, relocation processing, symbol binding, stack/auxv setup for dynamic launch, control transfer, and dynamic execution remain unavailable.
...
Proposed next milestone: M74 `dynamic relocation trace`. M74 should keep the M73 mapped app/interpreter proof, enumerate the app `DT_RELA`/`DT_JMPREL` relocation tables, report relocation counts/types/first target, and deny before applying relocations or transferring control.

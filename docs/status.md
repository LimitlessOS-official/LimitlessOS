...
Proposed next milestone: M42 `localbin dotdot relative path proof`. M42 should trace `linux /APPS/BUSYBOX sh -c 'cd /nvme/apps/data; /usr/local/bin/sbcat ../data/file.txt | /usr/local/bin/sbcat'` and make the minimum change needed, if any, so absolute-localbin third-party utilities inherit cwd and resolve `..` segments in cwd-relative paths through forked child `execve`.

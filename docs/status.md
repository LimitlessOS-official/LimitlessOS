...
M100 is `dynamic fcntl descriptor duplication breadth`. The UEFI Product Linux persona launcher now runs an eighteenth dynamic ET_EXEC artifact, `/APPS/DYNFDUP`, through the same bounded supported-interpreter path proven by M83-M99. The run proves an NVMe FAT-staged dynamic app can receive scoped NVMe VFS read authority, bind generated libc `fcntl`, `openat`, `read`, `close`, and `write` stubs, open a real VFS fd for `/nvme/apps/data/file.txt`, set `FD_CLOEXEC` on the original descriptor, duplicate it with `F_DUPFD`, confirm the duplicate does not inherit `FD_CLOEXEC`, read the same FAT-backed file through the duplicate descriptor, close both descriptors, print `dynfdup:dupfd:no-cloexec`, and release the VFS binding cleanly. Broader dynamic linking remains intentionally narrow: this is still a fixed supported-interpreter handoff with bounded relocations and the in-tree libc shim, not arbitrary shared-library loading.
...
Proposed next milestone: M101 `dynamic dup syscall descriptor duplication breadth`. M101 should expose and exercise direct generated libc wrappers for existing `dup(32)`, `dup2(33)`, and `dup3(292)` on brokered stdout and real VFS fds, prove exact-fd replacement and `O_CLOEXEC` behavior, and confirm the sidecar-copy fix from M100 works outside the `fcntl(F_DUPFD)` route without adding filesystem mutation or arbitrary dynamic linker search/loading.

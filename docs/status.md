...
M94 is `dynamic vectored I/O and readiness breadth`. The UEFI Product Linux persona launcher now runs a twelfth dynamic ET_EXEC artifact, `/APPS/DYNVEC`, through the same bounded supported-interpreter path proven by M83-M93. The run proves a boot-media-staged dynamic app can receive scoped NVMe VFS read authority, bind generated libc `readv`, `writev`, and `poll` syscall stubs, observe brokered stdout readiness with `POLLOUT`, read `/nvme/apps/data/file.txt` into multiple iovecs, print `dynvec:Nested:FAT32 path fixture` through vectored console output, and release the VFS binding cleanly. Broader dynamic linking remains intentionally narrow: this is still a fixed supported-interpreter handoff with bounded relocations and the in-tree libc shim, not arbitrary shared-library loading.
...
Proposed next milestone: M95 `dynamic fstatat metadata breadth`. M95 should expose the existing `newfstatat(262)` ABI through generated libc and run a dynamic ET_EXEC that uses `AT_FDCWD` and relative paths after `chdir` to verify file and directory metadata without adding `readlinkat`, write-side filesystem mutation, or arbitrary dynamic linker search/loading.
...
- UEFI checksum: `0x255B160B`
- UEFI SHA-256: `68136299ce6e66106b5684f5ae6476a6e3cd3f596c33e45616e2f38763657752`

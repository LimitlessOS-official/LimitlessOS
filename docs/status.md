...
M72 is `bounded interpreter file staging`. The UEFI Product Linux persona launcher still denies dynamic ELF before process allocation, but a supported `PT_INTERP` request can now be resolved to a staged interpreter candidate on the NVMe FAT image, read through the existing broker-scoped NVMe authority, and parsed for ELF metadata. Dynamic linking, relocation processing, interpreter mapping, control transfer, and dynamic execution remain unavailable.
...
Proposed next milestone: M73 `bounded interpreter mapping plan`. M73 should allocate a Linux persona for a supported dynamic input, map the app and staged interpreter `PT_LOAD` segments into the private process root, report app/interpreter mapped-page telemetry, then deny before relocations, symbol binding, or transfer of control.

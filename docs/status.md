...
M16 is `AI Policy Broker Foundation`.

M16 preserves M10 local login, M11 identity/vault behavior, M12 signed identity-provider descriptor verification, M13 account association, M14 cloud-storage broker policy/status, and M15 installer planning UX, then adds a Product AI policy foundation. UEFI Product records a system AI principal, action request, required consent, scope validation, denial result, and immutable/queryable audit status. M16 performs no AI action, model call, automation, system control, filesystem access, network access, settings mutation, package access, secret access, cloud access, or task execution.
...
- M16 has AI policy request/deny/audit only; AI assistant behavior, model integration, automation, task execution, cloud AI, and AI actions remain unavailable.

...
M68 is `bounded multi-page file-backed mmap proof`. The UEFI Product Linux persona path now accepts a regular-file `mmap` of an already-open NVMe VFS fd for the `MAP_PRIVATE` static-binary case up to the fixed 64 KiB `LINUX_ABI64_MMAP_FILE_COPY_BYTES` cap, eagerly copies file content into private anonymous VMAs, restores the requested page protection, and reports `mmap-file`, `mmap-file-bytes`, and `mmap-file-denial` telemetry. This is intentionally not full lazy file-backed VM yet: `MAP_SHARED`, writeback, page-fault population, mappings larger than 64 KiB, and BIOS support remain unavailable.

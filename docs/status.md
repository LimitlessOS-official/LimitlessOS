...
M85 is `dynamic libc helper breadth proof`. The UEFI Product Linux persona launcher now runs a third dynamic ET_EXEC artifact, `/APPS/DYNHELPER`, through the same bounded supported-interpreter path proven by M83-M84. The run proves the dynamic path transfers to the relocated application entry, maps the generated `libc-x64.so` shim in the live process root, applies nine relocations, binds `memcpy`, `puts`, `strcmp`, `strlen`, and `__libc_start_main`, prints visible brokered console output from the app's own `main`, exits through `exit_group(231)`, and leaves the process/root pools clean. Broader dynamic linking remains intentionally narrow: this is still a fixed supported-interpreter handoff with bounded relocations and the in-tree libc shim, not arbitrary shared-library loading.
...
Proposed next milestone: M86 `dynamic environment and stdio breadth`. M86 should run a dynamic ET_EXEC that exercises `getenv`, `printf`/`fputs`/`fwrite`, and inherited `argc`/`argv`/`envp` content through the same app-entry path, then either accept visible output with `exit 0` or capture the first precise failure stage/symbol/syscall/RIP as the next bounded dynamic-runtime target.
...
- UEFI checksum: `0x935CD0AB`
- UEFI SHA-256: `a542589f65779ca1ddc7baa6994c124f8777c02fa89dcf2aaf78c96951a52357`

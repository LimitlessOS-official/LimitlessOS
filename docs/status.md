...
M103 is `dynamic fork-plus-pipe composition`. The UEFI Product Linux persona launcher now runs a twenty-first dynamic ET_EXEC artifact, `/APPS/DYNFORKPIPE`, through the same bounded supported-interpreter path proven by M83-M102. The run proves an NVMe FAT-staged dynamic app can bind generated libc `fork` and `wait4` stubs alongside `pipe`, `read`, `write`, and `close`, create a pipe, fork a child, copy both pipe endpoints into the child fd table, have the child write through the inherited pipe write end, reap the child with `wait4`, read the child payload through the parent pipe read end, print `dynforkpipe:child-pipe`, and clean up both process roots and the pipe object. Broader dynamic linking remains intentionally narrow: this is still a fixed supported-interpreter handoff with bounded relocations and the in-tree libc shim, not arbitrary shared-library loading.
...
Proposed next milestone: M104 `blocked pipe read replay`. M104 should fix the observed parent-read-before-child-write ordering so a blocking pipe `read` resumes by completing the original syscall with copied bytes instead of returning an apparent failed read, then rerun the original DYNFORKPIPE ordering and prove `pipe-blocks 1`, `pipe-wakes 1`, `read 1`, `read-bytes 10`, `wait4-reap 1`, `pipe-live-final 0`, and `exit 0`.

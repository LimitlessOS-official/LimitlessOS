...
M80 is `safe relocation write/readback`. The UEFI Product Linux persona launcher still denies dynamic ELF before control transfer, but the supported `PT_INTERP` denial path now applies and reads back the five bounded apply-ready dynamic relocations while the target process CR3 is active. The unavailable `__libc_start_main` startup binding remains blocked, so real libc startup handoff, stack/auxv setup for dynamic launch, task registration, control transfer, and dynamic execution remain unavailable.
...
Proposed next milestone: M81 `bounded libc startup shim`. M81 should replace the unavailable `__libc_start_main` provider with a bounded Limitless libc startup handoff that can call a dynamic app's `main(argc, argv, envp)` and exit through `exit_group`, while still denying before general dynamic transfer until stack/auxv construction and startup invariants are proven.
...
- UEFI kernel bytes: 1,289,760
- UEFI kernel byte limit: 2,097,152 bytes
- UEFI byte reserve: 807,392

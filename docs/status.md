...
M69 is `bounded windowed file-backed mmap proof`. The UEFI Product Linux persona path now accepts a regular-file `mmap` of an already-open NVMe VFS fd for the `MAP_PRIVATE` static-binary case up to the fixed 64 KiB `LINUX_ABI64_MMAP_FILE_COPY_BYTES` mapping cap, but file population now uses a 4 KiB `LINUX_ABI64_MMAP_FILE_WINDOW_BYTES` range-read window instead of requiring the whole file to fit in one scratch buffer. This keeps eager private-copy semantics while allowing a small mapping at a high offset inside a larger file. `MAP_SHARED`, writeback, page-fault population, mappings larger than 64 KiB, and BIOS support remain unavailable.
...
Proposed next milestone: M62 `non-shell execvp repeated-slash directory executable denial proof`. M62 should exercise `/usr//local/bin` through third-party `sbenv` `execvp`, proving repeated-slash normalization still rejects a directory target before any backend binary read.

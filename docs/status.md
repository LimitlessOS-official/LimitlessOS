...
M82 is `dynamic stack and auxv frame proof`. The UEFI Product Linux persona launcher still denies dynamic ELF before task registration or execution, but the supported `PT_INTERP` denial path now constructs and validates a bounded initial dynamic launch stack for the mapped app/interpreter pair, including `argc`, `argv`, `envp`, base SysV `AT_*` auxv entries, rewritten `AT_RANDOM`/`AT_PLATFORM` stack addresses, and interpreter transfer RIP/RSP readiness. Dynamic task registration, dynamic-linker ABI handoff, and dynamic execution remain unavailable.
...
Proposed next milestone: M83 `dynamic task registration trace`. M83 should register a bounded dynamic Linux persona task using the proven interpreter RIP/RSP frame, transfer control under the normal scheduler/CR3/syscall boundary, and stop at the first real runtime fault or successful bounded console output without broadening the dynamic linker beyond the M80-M82 proof surface.
...
- UEFI kernel bytes: 1,293,856
- UEFI kernel byte limit: 2,097,152 bytes
- UEFI byte reserve: 803,296

...
M76 is `dynamic relocation support matrix`. The UEFI Product Linux persona launcher still denies dynamic ELF before relocation or control transfer, but the supported `PT_INTERP` denial path now keeps the M75 symbol trace and additionally walks all bounded RELA and PLT relocation entries, counts relocation types, and classifies each referenced symbol against the current Limitless libc/interpreter export registries. Dynamic relocation application, symbol binding, GOT/PLT writes, stack/auxv setup for dynamic launch, control transfer, and dynamic execution remain unavailable.
...
Proposed next milestone: M77 `weak relocation admission`. M77 should classify undefined weak `GLOB_DAT` symbols as safely nullable when the ELF ABI permits it, keep strong missing symbols denied, and prove the support matrix moves from five hard misses to the smaller set of symbols that truly require runtime providers before relocation application.

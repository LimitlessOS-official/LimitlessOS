...
M70 is `real dynamic ELF boundary telemetry`. The UEFI Product Linux persona launcher still denies dynamic ELF before process allocation, but the denial now records parsed ELF type, `PT_LOAD`, `PT_INTERP`, `PT_DYNAMIC`, and bounded `DT_NEEDED` analysis instead of reporting only `stage static code 8`. This makes the dynamic-linking boundary auditable with real externally built artifacts. Dynamic linking, relocation processing, interpreter handoff, and dynamic execution remain unavailable.
...
Proposed next milestone: M71 `PT_INTERP dynamic ELF boundary trace`. M71 should obtain or build a real dynamically linked artifact with `PT_INTERP`, stage the referenced interpreter metadata through NVMe provenance, and report the exact handoff boundary without executing relocations or claiming dynamic-linker support.

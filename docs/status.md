...
M17 is `AI Assistant Read-Only Mode`.

M17 preserves M10 local login, M11 identity/vault behavior, M12 signed identity-provider descriptor verification, M13 account association, M14 cloud-storage broker policy/status, M15 installer planning UX, and M16 AI request/deny/audit policy, then adds a Product Assistant host in Mode B. UEFI Product can open the Assistant app after login, request scoped read-only context through consent, deny sensitive or over-broad scopes with no data returned, grant only bounded read-only status context when allowed, and record the result in audit telemetry. M17 performs no AI action, model call, generated answer, automation, system control, filesystem write, settings mutation, package mutation, secret access, cloud access, or task execution.
...
- UEFI kernel bytes: 585536
- UEFI kernel byte limit: 2,097,152 bytes
- UEFI byte reserve: 1,511,616
...
- M17 has an Assistant host, consent prompt, read-only context grant, denial, and audit path only; inference backend, model transport, generated answers, automation, task execution, cloud AI, and AI actions remain unavailable.

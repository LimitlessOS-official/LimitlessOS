...
M87 is `dynamic heap and environment mutation`. The UEFI Product Linux persona launcher now runs a fifth dynamic ET_EXEC artifact, `/APPS/DYNHEAPENV`, through the same bounded supported-interpreter path proven by M83-M86. The run proves the dynamic path can bind and execute the generated libc shim's bounded heap helpers (`malloc`, `calloc`, `realloc`, and `free`) plus mutable environment helpers (`getenv` and `setenv`), prints visible brokered console output from the app's own `main`, exits through `exit_group(231)`, and leaves the process/root pools clean. Broader dynamic linking remains intentionally narrow: this is still a fixed supported-interpreter handoff with bounded relocations and the in-tree libc shim, not arbitrary shared-library loading.
...
Proposed next milestone: M88 `dynamic pthread helper smoke`. M88 should run a dynamic ET_EXEC that exercises the generated libc shim's pthread helper surface on top of the existing M64-M66 thread/futex/TLS foundation, starting with `pthread_create`, `pthread_join`, `pthread_mutex_lock`, and `pthread_mutex_unlock`, then either accept visible output with `exit 0` or capture the first precise helper/syscall/RIP failure.
...
- UEFI checksum: `0x44EF6F18`
- UEFI SHA-256: `0d2f1c2724553ecad1a01dbd300216ea2ab004d0ec59307580b975b133fbb4ad`

...
M98 is `dynamic fchdir directory-fd cwd handoff`. The UEFI Product Linux persona launcher now runs a sixteenth dynamic ET_EXEC artifact, `/APPS/DYNFCHDIR`, through the same bounded supported-interpreter path proven by M83-M97. The run proves an NVMe FAT-staged dynamic app can receive scoped NVMe VFS read authority, bind generated libc `fchdir`, `openat`, `read`, `close`, and `write` stubs, open `/nvme/apps/data` as a real directory fd, call `fchdir(datafd)`, use cwd-relative `openat(AT_FDCWD, "file.txt", O_RDONLY, 0)`, read the file content, print `dynfchdir:Nested:FAT32`, and release the VFS binding cleanly. Broader dynamic linking remains intentionally narrow: this is still a fixed supported-interpreter handoff with bounded relocations and the in-tree libc shim, not arbitrary shared-library loading.
...
Proposed next milestone: M99 `dynamic fcntl descriptor flag breadth`. M99 should expose the existing `fcntl(72)` ABI through the generated libc shim and run a dynamic ET_EXEC that uses `F_GETFD`, `F_SETFD`, `F_GETFL`, and the currently supported `F_SETFL` subset on a real VFS fd, proving descriptor/status flag translation without adding new filesystem mutation or arbitrary dynamic linker search/loading.

...
M75 is `dynamic relocation symbol trace`. The UEFI Product Linux persona launcher still denies dynamic ELF before relocation or control transfer, but the supported `PT_INTERP` denial path now keeps the M74 relocation-table census and additionally parses `DT_SYMTAB`, `DT_STRTAB`, `DT_STRSZ`, and `DT_SYMENT` to report the first needed dependency plus the symbol names referenced by the first bounded RELA and PLT relocations. Dynamic relocation application, symbol binding, GOT/PLT writes, stack/auxv setup for dynamic launch, control transfer, and dynamic execution remain unavailable.
...
Proposed next milestone: M76 `dynamic relocation support matrix`. M76 should walk all bounded RELA and PLT relocation entries, count relocation types and referenced symbol names, classify which symbols are backed by the current Limitless libc/interpreter export surface, report supported/missing binding counts, and continue denying before applying relocations.
...
- UEFI kernel bytes: 1,285,664
- UEFI kernel byte limit: 2,097,152 bytes
- UEFI byte reserve: 811,488

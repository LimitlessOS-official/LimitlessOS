...
Proposed next milestone: M56 `non-shell execvp trailing-slash executable denial proof`. M56 should exercise a slash-suffixed executable path requested by a third-party `sbenv` process through `execvp` and prove the M51 trailing-slash denial semantics hold outside BusyBox ash itself, with no unintended NVMe backend reads, pipe leaks, PML4 leaks, or CR3 repair.

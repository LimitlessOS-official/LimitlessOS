...
M7 is `Signed Package System`.

M7 keeps the accepted Product surface from M6 and adds signed package admission for the UEFI Product kernel. It verifies Ed25519 signatures for the generated bootstrap package archive and payloads, verifies a signed update-index fixture with anti-rollback telemetry, and preserves scoped install/update authority. It does not add auto-install, an app store, package-manager UI, AI behavior, browser behavior, or real internal-disk install writes.
...
- UEFI kernel bytes: 528608
- UEFI kernel byte limit: 2,097,152 bytes
- UEFI byte reserve: 1,568,544
...
- admission checks: signed archive, signed payload, checksum match, scoped install token, owner, stale token, and capability policy
- denial telemetry: missing signature, invalid signature, checksum mismatch, wrong owner, stale token, rollback index, and no ambient install/update
- update check: signed local update-index fixture verified through Product telemetry; no auto-install and no Product package-manager UI
...
- M7 signed package admission exists, but package-manager UI, app store, auto-install, live public update fetching, and trusted-time expiry enforcement remain unavailable.

...
M67 is `bounded file-backed mmap proof`. The UEFI Product Linux persona path now accepts a regular-file `mmap` of an already-open NVMe VFS fd for the minimal `MAP_PRIVATE` static-binary case, eagerly copies up to one page of file content into a private anonymous VMA, restores the requested page protection, and reports `mmap-file`, `mmap-file-bytes`, and `mmap-file-denial` telemetry. This is intentionally not full lazy file-backed VM yet: `MAP_SHARED`, writeback, page-fault population, mappings larger than 4 KiB, and BIOS support remain unavailable.

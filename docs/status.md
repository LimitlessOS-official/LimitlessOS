...
M81 is `bounded libc startup shim`. The UEFI Product Linux persona launcher still denies dynamic ELF before control transfer, but the supported `PT_INTERP` denial path now provides a real bounded `__libc_start_main` shim in the in-tree Limitless libc image, resolves that startup binding, applies all six bounded relocations, and reads them back while the target process CR3 is active. Dynamic stack/auxv setup, task registration, control transfer, and dynamic execution remain unavailable.
...
Proposed next milestone: M82 `dynamic stack and auxv frame proof`. M82 should construct and validate the initial dynamic launch stack for the mapped app/interpreter pair, including `argc`, `argv`, `envp`, and bounded `AT_*` auxv entries, then report transfer RIP/RSP without registering or running the dynamic task.

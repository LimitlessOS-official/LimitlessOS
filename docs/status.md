...
M104 is `blocked pipe read replay`. The UEFI Product Linux persona launcher now runs the original parent-read-before-wait `/APPS/DYNFORKPIPE` dynamic ET_EXEC smoke through the same bounded supported-interpreter path proven by M83-M103. The run proves a parent process can block in `read(pipe)`, let the forked child run and write through the inherited pipe write end, wake the parent, replay the blocked syscall under the reader's own CR3, copy the pipe payload into the parent user buffer, then reap the child with `wait4`, print `dynforkpipe:child-pipe`, and clean up both process roots and the pipe object. Broader dynamic linking remains intentionally narrow: this is still a fixed supported-interpreter handoff with bounded relocations and the in-tree libc shim, not arbitrary shared-library loading.
...
Proposed next milestone: M105 `dynamic pipe close/error semantics`. M105 should trace and then prove the remaining pipe edge cases a dynamic libc user expects: EOF after all write ends close, `SIGPIPE`/`EPIPE` when writing after all read ends close, and no leaked blocked tasks or pipe objects across those close/error paths.

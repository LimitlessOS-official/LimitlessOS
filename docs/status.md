...
M95 is `dynamic fstatat metadata breadth`. The UEFI Product Linux persona launcher now runs a thirteenth dynamic ET_EXEC artifact, `/APPS/DYNFSTATAT`, through the same bounded supported-interpreter path proven by M83-M94. The run proves a boot-media-staged dynamic app can receive scoped NVMe VFS read authority, bind generated libc `newfstatat`, `chdir`, and `write` stubs, update cwd to `/nvme/apps`, use `newfstatat(AT_FDCWD, ...)` on relative file and directory paths, validate file size plus regular-file/directory mode metadata, print `dynfstatat:27:file-dir`, and release the VFS binding cleanly. Broader dynamic linking remains intentionally narrow: this is still a fixed supported-interpreter handoff with bounded relocations and the in-tree libc shim, not arbitrary shared-library loading.
...
Proposed next milestone: M96 `dynamic openat relative file-read breadth`. M96 should expose the existing `openat(257)` ABI through a generated libc wrapper that handles the same C ABI to Linux syscall ABI details, then run a dynamic ET_EXEC that uses `openat(AT_FDCWD, "data/file.txt", O_RDONLY, 0)` after `chdir`, reads the FAT-backed content, and closes the fd without adding filesystem mutation or arbitrary dynamic linker search/loading.

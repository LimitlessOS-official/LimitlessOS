...
M88 is `dynamic pthread helper smoke`. The UEFI Product Linux persona launcher now runs a sixth dynamic ET_EXEC artifact, `/APPS/DYNTHREAD`, through the same bounded supported-interpreter path proven by M83-M87. The run proves the dynamic path can bind and execute generated libc shim pthread helpers (`pthread_create`, `pthread_join`, `pthread_mutex_lock`, and `pthread_mutex_unlock`) on top of the existing clone/thread/wait/futex foundation, prints visible brokered console output from the app's own `main`, exits through `exit_group(231)`, and leaves thread, process-root, futex, and wait state clean. Broader dynamic linking remains intentionally narrow: this is still a fixed supported-interpreter handoff with bounded relocations and the in-tree libc shim, not arbitrary shared-library loading.
...
- UEFI checksum: `0x3E928B11`
- UEFI SHA-256: `fd2f1fc5df19035167dc73c8ab8ecd949a1c84b1dbdf6e08fe60d3d6f9b46bc8`

...
M18 is `AI Consent-Scoped Action Mode`.

M18 preserves M10 local login, M11 identity/vault behavior, M12 signed identity-provider descriptor verification, M13 account association, M14 cloud-storage broker policy/status, M15 installer planning UX, M16 AI request/deny/audit policy, and M17 Assistant read-only context flow, then adds a Product Assistant action broker foundation in Mode B. UEFI Product can request one of four deterministic action templates, require explicit consent, grant only the exact scoped capability needed, expire the grant after completion, and record audit telemetry. M18 performs no model call, generated answer, cloud AI, autonomous behavior, package install/update, settings mutation, secret/token access, cloud storage access, or internal install/write.

...
M101 is `dynamic dup syscall descriptor duplication breadth`. The UEFI Product Linux persona launcher now runs a nineteenth dynamic ET_EXEC artifact, `/APPS/DYNDUP`, through the same bounded supported-interpreter path proven by M83-M100. The run proves an NVMe FAT-staged dynamic app can receive scoped NVMe VFS read authority, bind generated libc `dup`, `dup2`, `dup3`, `fcntl`, `openat`, `read`, `close`, and `write` stubs, duplicate brokered stdout, duplicate real VFS fds by direct syscalls, replace an exact fd with `dup2`, set `FD_CLOEXEC` with `dup3(..., O_CLOEXEC)`, verify the flag with `fcntl(F_GETFD)`, read the same FAT-backed file through each duplicate, print `dyndup:dup:dup2:dup3`, and release the VFS binding cleanly. Broader dynamic linking remains intentionally narrow: this is still a fixed supported-interpreter handoff with bounded relocations and the in-tree libc shim, not arbitrary shared-library loading.
...
Proposed next milestone: M102 `dynamic pipe syscall breadth`. M102 should expose and exercise the direct generated libc wrapper for existing `pipe(22)` from a dynamic ET_EXEC artifact, prove read/write/close over a kernel pipe fd pair without filesystem mutation, and only then decide whether to broaden into dynamic fork-plus-pipe composition.

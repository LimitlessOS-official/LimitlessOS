...
M96 is `dynamic openat relative file-read breadth`. The UEFI Product Linux persona launcher now runs a fourteenth dynamic ET_EXEC artifact, `/APPS/DYNOPENAT`, through the same bounded supported-interpreter path proven by M83-M95. The run proves a boot-media-staged dynamic app can receive scoped NVMe VFS read authority, bind generated libc `openat`, `chdir`, `read`, `close`, and `write` stubs, update cwd to `/nvme/apps`, use `openat(AT_FDCWD, "data/file.txt", O_RDONLY, 0)` on a relative FAT-backed path, read the file content, print `dynopenat:Nested:FAT32`, and release the VFS binding cleanly. Broader dynamic linking remains intentionally narrow: this is still a fixed supported-interpreter handoff with bounded relocations and the in-tree libc shim, not arbitrary shared-library loading.
...
Proposed next milestone: M97 `dynamic openat dirfd-relative lookup breadth`. M97 should keep the same bounded supported-interpreter path and prove `openat` can resolve a file relative to an already opened directory fd, for example opening `/nvme/apps/data` first and then reading `file.txt` with `openat(datafd, "file.txt", O_RDONLY, 0)`, without adding filesystem mutation, arbitrary dynamic linker search/loading, or new syscall surface unless the trace proves it is needed.

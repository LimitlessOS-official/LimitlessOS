...
Proposed next milestone: M58 `non-shell execvp bare-directory executable denial proof`. M58 should exercise the same directory target without the terminal slash, `/usr/local/bin`, through third-party `sbenv` `execvp`, proving pure directory-as-executable denial independently from trailing-slash intent.

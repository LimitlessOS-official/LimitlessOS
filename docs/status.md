...
M74 is `dynamic relocation table trace`. The UEFI Product Linux persona launcher still denies dynamic ELF before relocation or control transfer, but the supported `PT_INTERP` denial path now keeps the M73 bounded app/interpreter mapping proof and additionally enumerates the dynamic app relocation metadata for `DT_RELA`, `DT_RELASZ`, `DT_RELAENT`, `DT_JMPREL`, `DT_PLTRELSZ`, and `DT_PLTREL`. It reports relocation counts plus the first RELA and PLT relocation target/type, then cleans up the VMA/process-root state. Dynamic relocation application, symbol binding, GOT/PLT writes, stack/auxv setup for dynamic launch, control transfer, and dynamic execution remain unavailable.
...
Proposed next milestone: M75 `dynamic relocation symbol trace`. M75 should extend the M74 relocation census by parsing `DT_SYMTAB`, `DT_STRTAB`, and symbol indexes referenced by the first bounded `GLOB_DAT`/`JUMP_SLOT` relocations, report the required dynamic symbol names and source `DT_NEEDED` dependency, and continue denying before binding symbols or applying relocations.
...
- UEFI kernel bytes: 1,281,568
- UEFI kernel byte limit: 2,097,152 bytes
- UEFI byte reserve: 815,584

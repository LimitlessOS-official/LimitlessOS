...
The fix centralizes the remaining filter behavior without editing the long hardware-validation call list one row at a time. Decimal and hex field helpers now apply the same case-insensitive `shell64_hwval_label_matches_filter()` check when they are not part of an explicitly scoped composite line. The major `[x64] drs-*` composite summary rows now begin and end through a small shell-local composite-line wrapper, so an active filter checks the row prefix once and either emits the entire row or suppresses it as a unit. No xHCI, USB storage, FAT, input, compositor, Linux ABI, or BIOS behavior changed.

Accepted verification: Product x86_64 build completed, the M1 production-slice gate passed, and `tools\verify-qemu.ps1 -Architecture x86_64 -BootMedia uefi -BuildProfile Product -HardwareDisplayGate` passed. BIOS reserve stayed at `101` sectors, and UEFI reserve is `677120` bytes. Source checks also confirmed no raw `[x64] drs-*` composite prefixes remain in `shell64_print_hardware_validation_status()` without the composite filter wrapper.

M188 is `Product scroll and resize redraw containment`. Investigation traced the resize path through `display64_wm_process_mouse_event()`: resize motion calls `display64_wm_resize_window()`, which already marks the old and new window rectangles dirty, but then immediately called full `display64_desktop_redraw()`, expanding the next physical present to the whole framebuffer. The wheel path in `display64_wm_process_mouse_wheel()` had the same class of redraw bug in a different form: Settings, Terminal, File Manager, and Installer scroll state changed only one target window, but each branch presented the whole desktop.

The fix keeps the existing app behavior intact and changes only presentation scope. Resize now uses the dirty region already produced by `display64_wm_resize_window()`. Scroll branches mark the affected window dirty, rebuild the complete desktop/window stack into the compositor back buffer offscreen, restore the bounded dirty rectangle, and present only that window region. This is intentionally a redraw/damage fix; it does not add new scroll semantics, alter input drivers, change file manager paging, or touch BIOS code. Accepted verification: Product x86_64 build completed, the M1 production-slice gate passed, and `tools\verify-qemu.ps1 -Architecture x86_64 -BootMedia uefi -BuildProfile Product -HardwareDisplayGate` passed. BIOS reserve stayed at `101` sectors, and UEFI reserve is `677120` bytes.

M187 is `Product window drag redraw containment`. Investigation traced drag motion through `display64_wm_process_mouse_event()`: each held-left mouse packet moved the window with `display64_wm_move_window()`, which correctly marked the old and new window rectangles dirty, but then immediately called full `display64_desktop_redraw()`. Because the full redraw clears the whole desktop background, it expands compositor dirty state to the entire framebuffer and forces a full-screen physical present on every drag tick. That is the concrete cause of drag flash/flicker and excess copy work.

The fix adds a dirty-preserving desktop redraw helper for drag motion only. It snapshots the already-marked dirty region, rebuilds the full desktop/window stack into the compositor back buffer offscreen, restores the original dirty rectangle, and presents only that bounded old/new-window union. This keeps correctness for overlapping windows while avoiding full-screen visible updates during drag. Resize, scroll, and other redraw paths are intentionally left for their own trace/fix passes. Accepted verification: Product x86_64 build completed and the M1 production-slice gate passed. BIOS reserve stayed at `101` sectors, and UEFI reserve is `677120` bytes.

M186 is `Product cursor click artifact cleanup`. Investigation traced the cursor path in `display.c`: cursor movement and button-state changes are drawn directly onto the physical framebuffer using a saved-underlay cursor overlay, while the desktop/window contents are rebuilt through the compositor back buffer and dirty presentation. The concrete redraw ordering gap was `display64_desktop_redraw()` starting a full desktop/window rebuild without first restoring any currently drawn cursor overlay. Click transitions can redraw controls and focus state immediately after a cursor button-color update, so stale direct-drawn cursor pixels could survive as visual debris until a later cursor movement.

The fix is deliberately minimal: `display64_desktop_redraw()` now restores the saved cursor underlay before rebuilding the background, windows, launcher/context menus, and taskbar, then the normal compositor present path saves and draws the cursor again. This keeps the existing back-buffer, dirty-rect, cursor telemetry, and capability model intact; it does not change mouse input, hit testing, window manager semantics, xHCI/PS2/I2C drivers, or BIOS behavior. Accepted verification: Product x86_64 build completed and the M1 production-slice gate passed. BIOS reserve stayed at `101` sectors, and UEFI reserve is `681216` bytes.

M185 is `Product shell command history recall`. The persistent ring-3 Product shell now keeps a bounded 16-entry command history in its own fixed stack frame and handles broker-normalized up/down arrow escape sequences directly in the shell input loop. Up-arrow recalls older submitted command lines; repeated up-arrow walks backward through the ring; down-arrow walks forward and returns to the saved in-progress line or a blank line at the end. Repaint uses the existing console backspace erase sequence before writing the recalled command, so the implementation does not depend on unsupported carriage-return or terminal-control behavior.

The change is deliberately scoped to the interactive Product shell runtime (`runtime_image_user.asm`) rather than the kernel shell executor. This is the layer that already owns the console capability, echoes typed bytes, keeps the current command buffer, and calls `SHELL_EXECUTE_LINE`; the kernel `input64_read_keyboard_line` path remains unchanged for login/auth and other line-mode consumers. The history buffer is static and bounded inside the ring-3 shell stack frame (`16` lines x `128` bytes plus metadata), with no heap, no dynamic allocation, and no new ambient input or console authority. Accepted verification: Product x86_64 build completed, the M1 production-slice gate passed, and `tools\verify-qemu.ps1 -Architecture x86_64 -BootMedia uefi -BuildProfile Product -HardwareDisplayGate` passed. BIOS reserve stayed at `101` sectors, and UEFI reserve is `681216` bytes. The existing QEMU verifier can inject text lines but does not currently expose an arrow-key script parameter, so this milestone does not falsely claim automated up-arrow coverage beyond the boot/build regression gate.
...
New `hwval full` proof fields expose the selected interface and interrupt-transfer outcome: `xhci hid interfaces`, `xhci boot mouse interface`, `xhci boot mouse port`, `xhci boot mouse dci`, `xhci boot mouse mps`, `xhci boot mouse configured`, `xhci mouse last completion`, and raw mouse transfer event dwords `xhci mouse last event dw0` through `dw3`. Accepted verification: Product x86_64 build completed, M1 production-slice gate passed, BIOS reserve stayed at `101` sectors, UEFI reserve is `705664` bytes, `tools\verify-hardware-display-input-fixtures.ps1` passed `39/39`, and `tools\verify-qemu.ps1 -Architecture x86_64 -BootMedia uefi -BuildProfile Product -HardwareDisplayGate` passed while showing `xhci hid interfaces 2`, `xhci boot mouse interface 0`, `xhci boot mouse port 6`, `xhci boot mouse dci 3`, `xhci boot mouse mps 4`, `xhci boot mouse configured yes`, `xhci mouse last completion 1`, `xhci mouse reports 2`, and `mouse packets 2`. M183 does not claim the physical MSI pointer is fixed; the next physical run must prove whether the boot mouse interface is now selected and whether interrupt completions arrive on the real Intel xHCI controller.

M182 is `xHCI Address Device port diagnostics`. The UEFI Product xHCI path now preserves the raw Address Device command completion event when the command fails, recording all four event TRB dwords for `hwval full` as `xhci last address event dw0`, `dw1`, `dw2`, and `dw3`. It also snapshots the addressed port's `PORTSC` immediately before command submission and immediately after completion, including both the raw register and decoded Port Link State bits as `xhci pre-address portsc raw/pls` and `xhci post-address portsc raw/pls`. This is diagnostics-only: it does not change slot capacity, command submission, port reset policy, descriptor matching, retry counts, or any device-specific identity handling.

Accepted verification: Product x86_64 build completed, M1 production-slice gate passed, BIOS reserve stayed at `101` sectors, UEFI reserve is `710048` bytes, `tools\verify-hardware-display-input-fixtures.ps1` passed `39/39`, and `tools\verify-qemu.ps1 -Architecture x86_64 -BootMedia uefi -BuildProfile Product -HardwareDisplayGate` passed. QEMU `hwval full` showed the new fields with `xhci last address event dw0/dw1/dw2/dw3` all `0x00000000`, `xhci pre-address portsc raw: 0x00000E03`, `xhci pre-address portsc pls: 0`, `xhci post-address portsc raw: 0x00000E03`, `xhci post-address portsc pls: 0`, and `xhci address failures: 0`.

M181 is `xHCI slot capacity and hardware input diagnostics`. The UEFI Product xHCI path now allows 16 device slots while keeping the BIOS path at its existing 8-slot limit. Failed cleanup no longer burns the full command poll budget for every Disable Slot attempt; the UEFI cleanup path uses a bounded disable-slot poll limit of `10000`, and `hwval full` now reports the xHCI slot limit, disable poll limit, last Enable Slot completion/slot, last Address Device completion/slot/port/speed, address failure count, and last Disable Slot completion/slot. This is hardware-neutral controller bring-up work: it does not hardcode the MSI laptop, Logitech receiver, ELAN touchpad, USB port, ACPI ID, or any one-machine descriptor.

Accepted verification: Product x86_64 build completed, M1 production-slice gate passed, BIOS reserve stayed at `101` sectors, UEFI reserve is `710304` bytes, `tools\verify-hardware-display-input-fixtures.ps1` passed `39/39`, and `tools\verify-qemu.ps1 -Architecture x86_64 -BootMedia uefi -BuildProfile Product -HardwareDisplayGate` passed. QEMU `hwval full` showed `xhci max slots: 16`, `xhci disable slot poll limit: 10000`, `xhci last enable completion: 1`, `xhci last address completion: 1`, `xhci address failures: 0`, `xhci slots disabled: 0`, and `xhci slot disable failures: 0`.

M180 is `Windows hwval equivalent`. The repo now has a host-side `tools\hwval-windows.ps1` command that produces a LimitlessOS-style hardware validation transcript from the Windows PnP/device graph. It can read an existing Windows hardware inventory bundle or collect a fresh one with `-Collect`, then prints read-only policy, machine/firmware identity, hardware counts, a `drs-win-hwval` proof line, and focused Input, Storage, Display, and Network sections. The output is intentionally a Windows equivalent of `hwval`, not a LimitlessOS runtime transcript: it does not claim kernel packet counters, framebuffer draw counters, NVMe queue state, or Linux persona execution. Instead it reports the hardware facts Windows can prove better than LimitlessOS currently can: PCI IDs, ACPI IDs, HID collections/usages, services, compatible IDs, and the next generic driver target.

Accepted verification used `tools\verify-hwval-windows.ps1 -BundlePath .\dist\windows-hardware-inventory\windows-hardware-inventory-20260705-081144.zip -OutputDir .\build\windows-hwval-verification -RequireMsiCyborgEvidence`. The verified transcript includes `drs-win-hwval product 1 readonly 1 windows-source 1 relevant 189 usb-mouse 23 touchpad 7 i2c 3 xhci 1 storage 10 display 10 network 31`, Intel xHCI `PCI\VEN_8086&DEV_51ED`, Logitech receiver mouse collection `HID\VID_046D&PID_C53F&MI_01&COL01`, Intel Serial IO I2C host `PCI\VEN_8086&DEV_51E8`, ELAN touchpad `ACPI\ELAN0307` / `PNP0C50`, Intel RST/VMD `PCI\VEN_8086&DEV_A77F`, Micron NVMe disk behind that VMD path, Intel UHD Graphics `PCI\VEN_8086&DEV_A7A8`, and Intel Wi-Fi 6E AX211 `PCI\VEN_8086&DEV_51F1`. No kernel or ISO image was rebuilt for M180.

M179 is `Windows hardware inventory evidence verification`. This is host-side hardware evidence intake, not kernel driver enablement. The generic Windows inventory collector and summarizer can now use the already-working Windows device graph to classify real hardware without requiring repeated LimitlessOS USB flashes. The summarizer now handles single-object PowerShell results correctly, so scalar controller matches no longer produce blank counts, and the report surfaces primary USB/xHCI/touchpad/storage/display device identities for driver bring-up. The new verifier reruns the summary from a captured zip and asserts that the evidence contains nonzero USB mouse, touchpad, storage, and display targets; for the current physical laptop capture it also checks the expected MSI evidence is present.

Accepted verification used `dist\windows-hardware-inventory\windows-hardware-inventory-20260705-081144.zip` and passed `tools\verify-windows-hardware-inventory-summary.ps1 -BundlePath .\dist\windows-hardware-inventory\windows-hardware-inventory-20260705-081144.zip -OutputDir .\build\windows-hardware-inventory-summary-verification -RequireMsiCyborgEvidence`. The verified capture reports `189` relevant devices, `23` USB/HID mouse-class devices, `7` touchpad/HID pointer-class devices, `3` I2C controllers, `1` xHCI controller, `10` storage devices, `10` display devices, and `31` network devices. The concrete universal-driver fixtures are Intel xHCI `PCI\VEN_8086&DEV_51ED`, Logitech LIGHTSPEED receiver `USB/HID\VID_046D&PID_C53F` with a mouse collection at `MI_01&COL01`, ELAN I2C HID touchpad `ACPI\ELAN0307` compatible with `PNP0C50` behind Intel Serial IO I2C `PCI\VEN_8086&DEV_51E8`, Intel RST/VMD storage controller `PCI\VEN_8086&DEV_A77F` above the Micron NVMe disk, and Intel UHD Graphics `PCI\VEN_8086&DEV_A7A8`. These IDs are recorded as fixtures and evidence, not as hardcoded one-machine driver policy.

The next driver work should use this evidence to stop guessing from photos: for USB pointer support, improve xHCI composite HID interface matching/report acquisition around ordinary HID mouse collections such as `VID_046D&PID_C53F&MI_01&COL01`; for touchpad support, replace hardcoded I2C HID address probing with ACPI/PNP0C50 resource discovery; for internal storage, add a generic VMD/RST path before claiming internal NVMe availability. No kernel or ISO image was rebuilt for M179, so BIOS and UEFI reserves are unchanged from the last Product build.

M178 is `pointer enumeration evidence surface`. This responds to source-level review of the physical mouse pipeline: USB HID mouse reports and I2C HID touchpad samples already feed the shared brokered mouse queue, but physical MSI evidence still shows neither device reaching that downstream path. The UEFI Product `hwval full` output now exposes the missing upstream facts directly instead of depending on the suppressed desktop mouse overlay or serial-only hints. For xHCI, `hwval full` now includes `xhci mouse probe configured`, an explicit alias for the mouse endpoint being matched and configured. For I2C HID pointers, it now prints the captured pointer kind, I2C address, HID descriptor register, report descriptor register/length, input register, command register, max input length, and report-ID status.

This is hardware-neutral diagnostic hardening, not a device-specific mouse fix. It does not hardcode MSI, Logitech, ELAN, an I2C address, a USB port, or a descriptor shape, and it does not claim that the physical laptop mouse or touchpad now works. It makes the next laptop capture answer the concrete blocker questions: whether the USB dongle ever configures as a mouse probe, and which I2C HID address/register tuple was actually discovered or whether all pointer fields stayed zero while PCI/LPSS candidates existed.

Accepted verification: Product x86_64 build passed the M1 production-slice gate, BIOS reserve stayed at `101` sectors, UEFI reserve is `710,976` bytes, `tools\verify-hardware-display-input-fixtures.ps1` passed `39/39`, and `tools\verify-qemu.ps1 -Architecture x86_64 -BootMedia uefi -BuildProfile Product -HardwareDisplayGate` passed. QEMU proof showed `xhci mouse probe configured: yes`; because QEMU has no LPSS/I2C touchpad, the new I2C pointer address/register fields correctly reported zeroes. BIOS behavior is unchanged for this pass.

M177 is `Product system chrome readiness bar`. This continues the desktop polish track by replacing the purely flat top strip with UEFI Product system chrome: a `LimitlessOS` brand mark, Product badge, and live readiness chips for Display, Input, Storage, and Network. The chips are backed by existing hardware-neutral kernel state (`display64_readable`, brokered input packet/report counters, NVMe FAT availability, and hardware registry network count). There is no MSI-specific, Logitech-specific, ELAN-specific, or one-machine path in this milestone, and it does not claim new mouse/touchpad/storage/network hardware support.

The `drs-ui-polish` proof now carries `product-chrome 1`, and `tools\verify-qemu.ps1` accepts that optional field while preserving the older M109 telemetry shape. This keeps the Product gate strict without treating a new cosmetic proof field as a missing UI-polish regression. The change is intentionally small: it improves the first visual read of the desktop and gives normal users a cleaner system status surface while leaving raw `hwval full` evidence available for hardware bring-up.

Accepted verification: Product x86_64 build passed the M1 production-slice gate, BIOS reserve stayed at `101` sectors, UEFI reserve is `711,200` bytes, `tools\verify-hardware-display-input-fixtures.ps1` passed `39/39`, and `tools\verify-qemu.ps1 -Architecture x86_64 -BootMedia uefi -BuildProfile Product -HardwareDisplayGate` passed with `product-chrome 1`. BIOS behavior is unchanged for this pass.

M176 is `Product diagnostic overlay suppression`. This continues the Product desktop cleanup by removing the always-on debug feel from normal GUI use without hiding hardware evidence from verifiers. After the UEFI Product desktop is active, the boot/input diagnostics panel and mouse diagnostics panel no longer repaint over the right side of the desktop, File Manager, Settings, or Terminal. The raw evidence remains available through `hwval full`; the change only suppresses automatic overlay drawing after the compositor/desktop has taken ownership of the screen.

New `drs-gui` telemetry records `input-diag-suppressed` and `mouse-diag-suppressed`, proving that diagnostic paint requests were intentionally suppressed rather than silently lost. QEMU Product evidence reported `input-diag-suppressed 32` and `mouse-diag-suppressed 41` while retaining full `drs-display-readability`, `drs-ui-polish`, `drs-cursor-path`, storage, xHCI, and NVMe telemetry through `hwval full`. This milestone is hardware-neutral and universal-path only: it does not hardcode the MSI Cyborg, Logitech receiver, ELAN touchpad, or any specific machine, and it does not claim new pointer-driver support. It makes the Product GUI behave more like an operating system desktop instead of a permanent diagnostic overlay.

Accepted verification: Product x86_64 build passed the M1 production-slice gate, BIOS reserve stayed at `101` sectors, UEFI reserve is `711,232` bytes, `tools\verify-hardware-display-input-fixtures.ps1` passed `39/39`, and `tools\verify-qemu.ps1 -Architecture x86_64 -BootMedia uefi -BuildProfile Product -HardwareDisplayGate` passed with the new suppression counters. BIOS behavior is unchanged for this pass.

M175 is `Product storage/readiness visual hierarchy`. This continues the Product GUI cleanup without claiming new hardware-driver support. The UEFI Product File Manager now replaces raw unavailable/empty storage rows with a real storage status surface: an OK/WAIT status card, live NVMe FAT availability, real directory entry count, and scoped write-authority detail. Non-empty directories keep their real FAT-backed rows while gaining a compact storage status line above the listing, so the user sees what is actually mounted and what authority exists before invoking mutations.

Settings now has a more resilient Product readiness surface. The Display/Input/Storage/Network readiness strip renders as the full four-pill row on wide panels and as a compact two-column grid on narrow panels, with the Lock/action controls moved beneath the readiness area instead of overlapping it. The `drs-gui` proof line now carries `settings-readiness-strip` and `fileman-storage-card` alongside the existing hardware/input panel fields. This milestone is hardware-neutral: it improves how the current universal driver-readiness state is presented, but it does not hardcode or certify the MSI Cyborg, Logitech receiver, ELAN touchpad, or any other single device. The legacy MSI hardware-validation shell wording remains unchanged because the M1 production-slice gate currently verifies that exact handoff text.

Accepted verification: Product x86_64 build passed the M1 production-slice gate, BIOS reserve stayed at `101` sectors, UEFI reserve is `711,264` bytes, `tools\verify-hardware-display-input-fixtures.ps1` passed `39/39`, and `tools\verify-qemu.ps1 -Architecture x86_64 -BootMedia uefi -BuildProfile Product -HardwareDisplayGate` passed. QEMU evidence includes the new GUI proof fields and `fileman-storage-card 1`. BIOS behavior is unchanged for this pass.

M174 is `universal Product hardware readiness UX`. This keeps the Product surface moving from raw diagnostics toward a clearer user-facing hardware view without making false hardware claims. The UEFI Product Settings app now has real hardware and input-readiness panels backed by live kernel state instead of static copy: hardware inventory count, drivers bound/deferred, keyboard readiness, pointer readiness, PS/2 fallback presence, xHCI presence, xHCI mouse endpoint status, and I2C pointer status. Settings also shows a compact Display/Input/Storage/Network readiness strip so the desktop communicates whether the machine is usable before the user has to read raw telemetry. The existing `drs-gui` proof now includes `settings-hardware-panel` and `settings-input-panel`, proving those panels were rendered from the real Product state.
...
The UEFI Product xHCI enumeration path now gives each connected port only two failed enumeration attempts per boot before later live rescans skip it, preventing repeated failed Disable Slot cleanup from consuming minutes on real hardware. New `hwval` fields expose the bounded behavior and preserve first-candidate evidence instead of only last-skip noise: `xhci port probe attempts`, `xhci port probe retry skips`, `xhci first hid port/class/subclass/protocol`, and `xhci first mouse port/class/subclass/protocol/mps`. This is generic USB controller hygiene; it does not hardcode the MSI, Logitech, or ELAN devices and it does not claim the mouse is fixed until reports reach the brokered pointer queue. Accepted verification: Product x86_64 build passed M1, BIOS reserve stayed `101` sectors, UEFI reserve is `716,032` bytes, `tools\verify-hardware-display-input-fixtures.ps1` passed `39/39`, and `tools\verify-qemu.ps1 -Architecture x86_64 -BootMedia uefi -BuildProfile Product -HardwareDisplayGate` passed while showing `xhci port probe attempts 2`, `xhci port probe retry skips 0`, `xhci first mouse port 6`, `xhci first mouse protocol 2`, `xhci first mouse mps 4`, `xhci mouse endpoint yes`, and `xhci mouse reports 2` on QEMU. The refreshed MSI handoff bundle at `dist\m133-msi-hardware-handoff-current` self-verifies with ISO SHA-256 `ea5268b6581127c7a0a69e9d0fb070876c938cfa5f389f3deb8a1f7568cbf733`, UEFI image SHA-256 `bfaef31767418ae469841634c2451314cabfeee764d144ad138a792f846445dc`, `/APPS/DYNLDLIMIT` SHA-256 `9f6eb9c05b3065d39bc59d24defe9361267b34cefd4de78f568ddb00497238fa`, and `/APPS/LDLIMIT` SHA-256 `6f713105878c30d817b7add4a7ed5d4ee8e01fb6eab2c80ba10acee059c72238`. The next MSI flash should confirm whether the slot storm collapses and whether the Logitech port now records a first mouse candidate.

M172 is `MSI Windows hardware inventory and xHCI slot cleanup`. This continues the universal hardware-support track using the MSI Cyborg as the first physical proof target, without hardcoding MSI-specific behavior. The new host-side Windows inventory collector captures present PnP devices, relevant USB/HID/storage/display/I2C metadata, driver bindings, storage layout, and optional SetupAPI tail logs so physical hardware facts can guide generic driver work instead of guessing from scrolled LimitlessOS telemetry. The July 2026 MSI capture confirms the relevant hardware shape: Intel xHCI `8086:51ED`, Logitech LIGHTSPEED receiver `046D:C53F` on USB port 8 with mouse collection under composite HID `MI_01`, ELAN0307/PNP0C50 I2C HID touchpad behind Intel Serial IO I2C `8086:51E8`, and Micron NVMe storage hidden behind Intel VMD `8086:A77F`.

The UEFI Product xHCI path now disables xHCI slots after failed post-enable enumeration attempts and exposes `xhci slots disabled` plus `xhci slot disable failures` through `hwval`. This is generic controller hygiene for real USB topologies with multiple non-target devices; it prevents failed descriptor/configuration probes from consuming controller slots before a later HID mouse interface can be considered. The BIOS path is intentionally byte-stable: the shared xHCI probe keeps the old BIOS log/return shape and the BIOS reserve remains `101` sectors. Accepted verification: Product x86_64 build passed M1, `tools\verify-hardware-display-input-fixtures.ps1` passed `39/39`, and `tools\verify-qemu.ps1 -Architecture x86_64 -BootMedia uefi -BuildProfile Product -HardwareDisplayGate` passed with readable display, visible cursor, xHCI mouse endpoint present, xHCI mouse reports `2`, `xhci slots disabled 0`, and `xhci slot disable failures 0` on QEMU. Final reserves: BIOS `101` sectors, UEFI `716,288` bytes.

M171 is `MSI boot-latency timing telemetry`. This responds to the physical MSI observation that the laptop takes roughly 3-5 minutes to reach the Product desktop while VirtualBox reaches it in under 2 minutes. The UEFI Product boot path now records and prints bounded tick durations for the suspected slow stages after PIT initialization: xHCI init, boot diagnostics, NVMe probe, display init, I2C HID init, timer wait, keyboard wait, mouse wait, login, lock probe, window-manager probe, desktop probe, virtio-net init, and PIT-to-shell total. The same fields are exposed through `hwval` as `boot ticks ...` lines so the timing evidence can be captured after the desktop appears instead of relying on scrolled boot text.

Host-side display/input analysis now preserves the boot timing fields in `input_state` and the text report. Accepted verification: Product x86_64 build with staged dynamic artifacts passed the M1 production-slice gate, BIOS reserve stayed at `101` sectors, UEFI reserve is `716,352` bytes, `tools\prepare-hardware-storage-evidence.ps1 -EvidenceDir .\dist\m133-msi-hardware-handoff-current -SkipBuild -SkipQemuGate` refreshed and self-verified the current MSI handoff bundle, and `tools\verify-hardware-display-input-fixtures.ps1` passed `39/39`. The refreshed bundle records ISO SHA-256 `619ffac1d528b499438631c7a17dcab07d94b1b1c786cc32ef1ba8f9a8131d41` and UEFI image SHA-256 `1940c39fdca70fc203f49ba68a38145b7c2e9c333e0f9e41c58c33669280ba34`. M171 does not claim the MSI boot is faster yet; it makes the next slow-boot capture identify the exact stage consuming the extra minutes.

M170 is `MSI xHCI mouse descriptor fallback and evidence`. This responds to the July 2026 MSI photos where the Product GUI reached the desktop with a visible but non-moving cursor, `hwval` showed `XHCI Y`, `HANDOFF Y`, `HID N`, and `USB2 12 ERR 27`, and both a USB mouse dongle and the built-in touchpad produced no usable pointer movement. The UEFI Product xHCI path now has a conservative broad mouse-probe fallback for non-hub, non-boot-keyboard interrupt-IN interfaces with mouse-sized packets, so vendor-specific or composite USB mouse receivers can be bound without pretending movement exists. Pointer movement still requires real interrupt reports decoded by the existing brokered mouse path. `hwval` now prints descriptor-level xHCI evidence: last skip port/code, last device class/subclass/protocol, last configuration length, last interface class/subclass/protocol, last endpoint max packet, and broad mouse probe count.

Host-side display/input analysis now preserves those fields and adds the explicit `xhci-mouse-descriptor-unmatched` stage for captures that still stop at xHCI error `27`. Accepted verification: Product x86_64 build with staged dynamic artifacts passed the M1 production-slice gate, BIOS reserve stayed at `101` sectors, UEFI reserve is `716,896` bytes, `tools\prepare-hardware-storage-evidence.ps1 -EvidenceDir .\dist\m133-msi-hardware-handoff-current -SkipBuild -SkipQemuGate` refreshed and self-verified the current MSI handoff bundle, `tools\verify-msi-hardware-handoff.ps1 -RequireStagedDynamicArtifacts -RequireGuiInteractionTelemetry` passed, `tools\start-msi-hardware-capture.ps1` generated a fresh session, and `tools\verify-hardware-display-input-fixtures.ps1` passed `39/39`. The refreshed bundle records ISO SHA-256 `d3942bb40ed3f571f3f7d4f1a1f557343e4365df686d13f3b9e7c3c51454736b` and UEFI image SHA-256 `dc86c45602a0e524693693ec70944b203fa8b4bf4decf96a0b61a6a1e27393c3`. A UEFI Product QEMU verifier attempt reached the shell and logged xHCI input live, but the harness timed out before a clean verifier result; no QEMU verification pass or physical MSI pointer pass is claimed until the new ISO is booted on the laptop.

M169 is `MSI PIC MASK boot-stage diagnostics`. This is a UEFI-only diagnostic handoff for the physical laptop freeze reported after the visible `PIC MASK` marker, not a claimed hardware fix. The Product boot path now prints the APIC/PIC/IRQ route state immediately after `PIC MASK` and adds fine-grained stage markers around the next suspected cliff points: `PIT INIT`, `PIT OK`, `SYSCALL INIT`, `SYSCALL OK`, `INT PROBES`, and `INT PROBES OK`. These markers are compiled only under `LIMITLESS_X64_UEFI_KERNEL`, so the BIOS path is not burdened by the extra diagnostics.

Accepted verification: Product x86_64 was rebuilt with staged `/APPS/DYNLDLIMIT` and `/APPS/LDLIMIT`, the M1 production-slice gate passed, and the current MSI handoff bundle at `dist\m133-msi-hardware-handoff-current` was refreshed and self-verified. The refreshed bundle records ISO SHA-256 `69e95ab98684eb84112497de603ed9160a4563a0db20b54bddbf42d2f380eec6`, UEFI image SHA-256 `b2cdf962eed8b831d71f590ed646dc4794959211cbc88d5b74ac47db063d13e0`, `/APPS/DYNLDLIMIT` SHA-256 `9f6eb9c05b3065d39bc59d24defe9361267b34cefd4de78f568ddb00497238fa`, and `/APPS/LDLIMIT` SHA-256 `6f713105878c30d817b7add4a7ed5d4ee8e01fb6eab2c80ba10acee059c72238`. `tools\verify-msi-hardware-handoff.ps1 -RequireStagedDynamicArtifacts -RequireGuiInteractionTelemetry` passed with `source2-required: 2` and `nvme-controller-snapshot-required: 1`, and `tools\start-msi-hardware-capture.ps1` generated a fresh session pointing at the diagnostic ISO. Final reserves: BIOS `101` sectors and UEFI `721,280` bytes. M169 does not claim the MSI laptop boots past the freeze, fixes the mouse cursor, or certifies display/input; it makes the next physical run identify the first exact boot-stage cliff.

M168 is `MSI hardware capture finish helper`. This is host-side post-capture intake hardening, not new kernel behavior. The new `tools\finish-msi-hardware-capture.ps1` command defaults to the current handoff bundle and `dist\msi-hwval-storage.txt`, rejects an empty capture or the blank session template, runs `tools\report-msi-hardware-capture.ps1` with `-RequireStagedDynamicArtifacts -RequireGuiInteractionTelemetry`, accepts report exit `0` or actionable exit `2`, writes `msi-hardware-capture-next-target.txt`, and prints the target kind, target stage, roadmap target, next target, and basic transcript-presence checks for `hwval`, `linux /APPS/DYNLDLIMIT`, `drs-nvme-triage`, and `drs-realbin`.

Accepted verification: the M134 storage target fixture generator passed with `m134-storage-target-fixtures: 17/17`; `tools\finish-msi-hardware-capture.ps1` was run against the generated `storage-ready` synthetic capture and produced `target-kind: storage-ready`, `target-stage: storage-ready`, `roadmap-target: M134`, and report exit `0`; the blank template rejection path was also verified. M168 does not claim a physical MSI pass, a working VMD-backed NVMe controller, successful internal SSD FAT access, or safe internal-disk writes; it gives the real capture a single strict intake command once the transcript exists.

M167 is `MSI hardware capture session helper`. This is host-side operator-error hardening for the physical MSI run, not new kernel behavior. The new `tools\start-msi-hardware-capture.ps1` command verifies the current `dist\m133-msi-hardware-handoff-current` bundle with `tools\verify-msi-hardware-handoff.ps1 -RequireStagedDynamicArtifacts -RequireGuiInteractionTelemetry`, creates a capture-session output directory, writes `msi-hardware-capture-session.txt` with the exact handoff ISO path, bundle hashes, reserves, laptop commands, required telemetry lines, capture path, and report command, and writes a blank transcript template without fabricating hardware evidence. If `-AnalyzeIfCaptureExists` is supplied and a real capture file already exists, it runs the M155 report wrapper and accepts exit `0` or actionable exit `2`.

Accepted verification: `tools\start-msi-hardware-capture.ps1` against the M166 refreshed bundle, which reverified `source2-required: 2`, `nvme-controller-snapshot-required: 1`, BIOS reserve `101` sectors, and UEFI reserve `721,408` bytes. The helper wrote its ignored session files under `dist\msi-hardware-capture-session`. M167 does not claim a physical MSI pass, a working VMD-backed NVMe controller, successful internal SSD FAT access, or safe internal-disk writes; it makes the next real laptop run harder to mis-execute and easier to hand back as analyzable evidence.
...
`tools\analyze-msi-hardware-capture.ps1`, `tools\verify-msi-hardware-handoff.ps1`, `tools\classify-m134-storage-target.ps1`, and `tools\report-msi-hardware-capture.ps1` now carry that capture-side next target through the manager-facing report chain, so a stale transcript cannot be misread as a valid controller register snapshot. Fixture coverage now includes stale-capture cases at every layer: `m134-storage-target-fixtures: 17/17`, `msi-hardware-capture-report-fixtures: 4/4`, and `msi-hardware-handoff-fixtures: 17/17`. Product build plus the M1 production-slice gate passed. Final reserves remain BIOS `101` sectors and UEFI `721,408` bytes. M165 does not claim a physical MSI pass, a working VMD-backed NVMe controller, successful internal SSD FAT access, or safe internal-disk writes; it prevents stale physical transcripts from silently satisfying the current storage handoff contract.

M164 is `current MSI handoff bundle M163 refresh`. The ignored physical handoff bundle at `dist\m133-msi-hardware-handoff-current` has been regenerated from commit `ef31c577` with the M163 NVMe controller snapshot contract. The refreshed bundle records ISO SHA-256 `affbf09c3e7b460c090ea80c0473e6eed6623f76772db74f65bbcedecb8a8ec4`, UEFI image SHA-256 `ce92827618721dcdb326f2aebedc441e304ae9bfccfcf2d7c3f9bb955a47601a`, `/APPS/DYNLDLIMIT` SHA-256 `9f6eb9c05b3065d39bc59d24defe9361267b34cefd4de78f568ddb00497238fa`, and `/APPS/LDLIMIT` SHA-256 `6f713105878c30d817b7add4a7ed5d4ee8e01fb6eab2c80ba10acee059c72238`. The handoff verifier reports `source2-required: 2`, `nvme-controller-snapshot-required: 1`, and all nine required controller fields in `msi-hardware-handoff-verification.txt`. `tools\prepare-hardware-storage-evidence.ps1` now removes any stale `qemu-storage-stage-gate.txt` when `-SkipQemuGate` is used and writes `qemu-storage-stage-gate-skipped.txt` instead, so in-place bundle refreshes cannot carry old QEMU evidence as if it were current. Verification passed with the staged Product build plus M1 production-slice gate from the refresh attempt, then a `-SkipBuild -SkipQemuGate` bundle regeneration and self-verification. Final reserves remain BIOS `101` sectors and UEFI `721,408` bytes. M164 does not claim a physical MSI pass, a working VMD-backed NVMe controller, successful internal SSD FAT access, or safe internal-disk writes; it makes the currently handed-off USB package match the latest M163 intake contract.

M163 is `MSI handoff NVMe controller snapshot contract`. This is host-side handoff hardening for the M161/M162 controller register snapshot, not new kernel behavior. `tools\prepare-hardware-storage-evidence.ps1` now records `required_nvme_controller_snapshot = 1` plus the required controller fields in generated MSI handoff manifests: `nvme-probe-error`, `nvme-regs`, `nvme-cap-low`, `nvme-cap-high`, `nvme-vs`, `nvme-cc`, `nvme-csts`, `nvme-dstrd-bytes`, and `nvme-doorbell-page`. The generated `README-HARDWARE-STORAGE.txt` tells the tester to confirm those fields under `NVMe Controller Snapshot` in the report, and `tools\verify-msi-hardware-handoff.ps1` rejects stale bundles whose manifest or runbook omit the snapshot contract. Fixture bundle generators were updated so M134/M155/M162 report and handoff regressions exercise the same current contract. Verification passed with `m134-storage-target-fixtures: 16/16`, `msi-hardware-handoff-fixtures: 16/16`, `msi-hardware-capture-report-fixtures: 3/3`, and Product build plus M1 production-slice gate. Final reserves remain BIOS `101` sectors and UEFI `721,408` bytes. M163 does not claim a physical MSI pass, a working VMD-backed NVMe controller, successful internal SSD FAT access, or safe internal-disk writes; it makes future hardware handoff packages unable to silently drop the raw controller evidence needed to diagnose the next MSI storage failure.

M162 is `MSI NVMe controller snapshot report surfacing`. This is host-side report hardening for the M161 controller register snapshot, not new kernel behavior. `tools\verify-hardware-storage-evidence.ps1` now promotes the M161 `key_fields` into a stable `capture_nvme_controller` object with padded hex formatting for CAP/VS/CC/CSTS, `tools\analyze-msi-hardware-capture.ps1` carries that object into combined MSI analysis, `tools\classify-m134-storage-target.ps1` writes it into `m134-storage-target.json/.txt/.md`, and `tools\report-msi-hardware-capture.ps1` surfaces it in the manager-facing report. Fixture coverage now asserts the controller snapshot survives the full analyzer -> evidence verifier -> MSI analyzer -> M134 classifier -> capture report chain. Verification passed with `hardware-storage-analysis-fixtures: 67/67`, `m134-storage-target-fixtures: 16/16`, and `msi-hardware-capture-report-fixtures: 3/3`; Product build plus M1 production-slice gate passed. Final reserves remain BIOS `101` sectors and UEFI `721,408` bytes. M162 does not claim a physical MSI pass, a working VMD-backed NVMe controller, successful internal SSD FAT access, or safe internal-disk writes; it makes the M161 raw controller snapshot visible in the report a manager or hardware tester will actually read.

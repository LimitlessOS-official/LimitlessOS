...
M97 is `dynamic openat dirfd-relative lookup breadth`. The UEFI Product Linux persona launcher now runs a fifteenth dynamic ET_EXEC artifact, `/APPS/DYNODIR`, through the same bounded supported-interpreter path proven by M83-M96. The run proves an NVMe FAT-staged dynamic app can receive scoped NVMe VFS read authority, bind generated libc `openat`, `read`, `close`, and `write` stubs, open `/nvme/apps/data` as a real directory fd, use `openat(datafd, "file.txt", O_RDONLY, 0)` to resolve a child path relative to that fd, read the file content, print `dynopenatdirfd:Nested:FAT32`, and release the VFS binding cleanly. Broader dynamic linking remains intentionally narrow: this is still a fixed supported-interpreter handoff with bounded relocations and the in-tree libc shim, not arbitrary shared-library loading.

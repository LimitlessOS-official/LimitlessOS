...
M78 is `libc startup provider trace`. The UEFI Product Linux persona launcher still denies dynamic ELF before relocation or control transfer, but the supported `PT_INTERP` denial path now admits the local `libc-x64.so` dependency alias and classifies `__libc_start_main` as an explicit unavailable libc provider instead of an unknown hard-missing symbol. Dynamic relocation application, GOT/PLT writes, real libc startup handoff, stack/auxv setup for dynamic launch, control transfer, and dynamic execution remain unavailable.
...
Proposed next milestone: M79 `dynamic relocation dry-run`. M79 should keep dynamic execution denied, but compute and validate the exact target/value pairs for `R_X86_64_GLOB_DAT` and `R_X86_64_JUMP_SLOT` without writing memory, proving that relocation application is now blocked by the intentional unavailable startup provider and write policy rather than by parsing, dependency lookup, or symbol-provider lookup.

...
M77 is `weak relocation admission`. The UEFI Product Linux persona launcher still denies dynamic ELF before relocation or control transfer, but the supported `PT_INTERP` denial path now distinguishes ELF ABI nullable undefined weak `R_X86_64_GLOB_DAT` bindings from hard missing bindings. Dynamic relocation application, GOT/PLT writes, strong symbol binding, stack/auxv setup for dynamic launch, control transfer, and dynamic execution remain unavailable.
...
Proposed next milestone: M78 `libc startup provider trace`. M78 should keep dynamic execution denied, but add a bounded provider classification for the remaining strong startup binding, especially `__libc_start_main`, and prove whether the next blocker is dependency aliasing, provider address selection, or relocation application itself.

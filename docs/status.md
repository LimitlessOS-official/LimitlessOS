...
Proposed next milestone: M51 `absolute localbin trailing-slash executable denial proof`. M51 should trace `linux /APPS/BUSYBOX sh -c '/usr/local/bin/sbenv/ USER=operator /usr/local/bin/sbenv|sbcat'` and verify the trailing slash on a file executable is denied cleanly without page faults, pipe leaks, or PML4 leaks.

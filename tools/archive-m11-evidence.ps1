...
$evidenceDir = Join-Path $distDir "m11-evidence-$timestamp"
...
function Get-InventoryPath
{
    ...
    return Join-Path $distDir ("limitlessos-x86_64.{0}.m11.json" -f $BuildProfile.ToLowerInvariant())
}
...
function Archive-ProfileArtifacts
{
    ...
    foreach ($milestone in @("m1", "m2", "m3", "m4", "m5", "m6", "m7", "m8", "m9", "m10", "m11")) {
        if ($milestone -eq "m1") {
...
function Update-M11InventoryEvidenceStatus
{
    ...
    $inventory.identityPanelVerified = $true
    $inventory.identityStatusReadonlyVerified = $true
    $inventory.identityMutationDeniedVerified = $true
    $inventory.vaultFoundationVerified = $true
    $inventory.vaultSecretReadDeniedVerified = $true
    $inventory.vaultSecretWriteDeniedVerified = $true
    $inventory.noPlaintextTokenVerified = $true
    ...
    $inventory | Add-Member -Force -NotePropertyName m11EvidenceDirectory -NotePropertyValue (Get-RepoRelativePath $evidenceDir)
    try {
...
Update-M11InventoryEvidenceStatus -GitStatusPath $gitStatusPath
...
$metadata = [ordered]@{
    milestone = "M11 Identity Foundation + Secrets Vault"
    ...
    identityVerifierStatus = "passed"
    activeAccountType = if ($productInventory) { $productInventory.activeAccountType } else { "local" }
    unavailableAccountTypes = if ($productInventory) { $productInventory.unavailableAccountTypes } else { @("personal", "enterprise") }
    secretVaultStatus = if ($productInventory) { $productInventory.secretVaultStatus } else { "foundation metadata only; no secret/token storage" }
    encryptedVaultStatus = if ($productInventory) { $productInventory.encryptedVaultStatus } else { "unavailable/non-product" }
    realSecretsOrTokensStored = if ($productInventory) { $productInventory.realSecretsOrTokensStored } else { $false }
    settingsIdentityPanelStatus = "read-only Product surface"
    shellIdentityCommandStatus = "not added in M11; Settings provides the read-only Product identity surface"
    noAmbientIdentityAuthority = "verified"
    noAmbientSecretAuthority = "verified"
    ...
    blockers = @(
        ...
        "personal and enterprise account login remain unavailable",
        "encrypted-at-rest secret storage remains unavailable",
        "cloud storage remains unavailable",
        "trusted network identity transport remains future M12 work"
    )
}

$metadataPath = Join-Path $evidenceDir "m11-evidence.json"
$metadata | ConvertTo-Json -Depth 16 | Set-Content -LiteralPath $metadataPath -Encoding UTF8

$summaryPath = Join-Path $evidenceDir "m11-evidence-summary.md"
@(
    "# M11 Evidence Summary",
    "",
    "- Milestone: M11 Identity Foundation + Secrets Vault",
    ...
    "- Active account type: $($metadata.activeAccountType)",
    "- Unavailable account types: $($metadata.unavailableAccountTypes -join ', ')",
    "- Vault: $($metadata.secretVaultStatus)",
    "- Encrypted vault: $($metadata.encryptedVaultStatus)",
    "- Real secrets/tokens stored: $($metadata.realSecretsOrTokensStored)",
    "- Settings identity panel: $($metadata.settingsIdentityPanelStatus)",
    "- Shell identity command: $($metadata.shellIdentityCommandStatus)",
    "- No ambient identity authority: $($metadata.noAmbientIdentityAuthority)",
    "- No ambient secret authority: $($metadata.noAmbientSecretAuthority)",
    "- Commands passed: $passedCount / $($records.Count)",
    "- No M12 work started."
) | Set-Content -LiteralPath $summaryPath -Encoding UTF8

Write-Host "M11 evidence archived:"

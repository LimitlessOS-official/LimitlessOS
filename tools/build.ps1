...
function Build-X64Scaffold
{
    ...
    $scaffoldSplitSources = @($commonSources | Where-Object { $_.Name -like "scaffold_*.c" })
    if ($scaffoldSplitSources.Count -lt 10) {
        throw "x86_64 scaffold split sources are incomplete."
    }
    ...
    $commonSources = @($commonSources | Where-Object { $_.Name -ne "installer_ux.c" })
    $commonSources = @($commonSources | Where-Object { $_.Name -ne "ai_policy.c" })
    $biosSources = @($commonSources | Where-Object {
            ($_.Name -ne "virtio_net.c") -and ($_.Name -ne "e1000e.c")
        })
    $biosSources = @($biosSources | Where-Object { $_.Name -ne "vma.c" })
    $biosSources = @($biosSources | Where-Object { $_.Name -ne "fd.c" })
    $biosSources = @($biosSources | Where-Object { $_.Name -ne "pipe.c" })
    $biosSources = @($biosSources | Where-Object { $_.Name -ne "persona.c" })
    $biosSources = @($biosSources | Where-Object { $_.Name -ne "persona_audit.c" })
    $biosSources = @($biosSources | Where-Object { $_.Name -ne "macos_abi.c" })
    $biosSources = @($biosSources | Where-Object { $_.Name -ne "macos_mach.c" })
    $biosSources = @($biosSources | Where-Object { $_.Name -ne "macos_dyld.c" })
    $biosSources = @($biosSources | Where-Object { $_.Name -ne "macos_cf.c" })
    $biosSources = @($biosSources | Where-Object { $_.Name -ne "macos_shim.c" })
    ...
    $biosSources = @($biosSources | Where-Object { $_.Name -ne "windows_registry.c" })
...
user-entry: x64 broker derives a tokenized ring-3 entry frame with RIP 0x41000010, RSP 0x40020000, selectors 0x33/0x2B, interrupt-masked RFLAGS 0x00000002, denial 0, and transfer-ready telemetry once separate user executable and stack views are installed without weakening the supervisor-only validation mapping, verifies controlled iretq entry, IF-enabled PIT frame capture, scheduler-owned switching, reusable scheduler_x64 saved-frame dispatch, brokered fs_read and second-page filesystem/display mutation probes at RIP 0x41001ED0, with command execution now sourced from disk-backed APPS descriptors and flat binaries through the launch broker
...
input: x64 brokered input syscall surface accepts only principal-scoped input service capabilities, validates writable user-stack or kernel-high buffers before copying command bytes, keeps the original byte-stream read path for compatibility, adds a line-oriented read syscall for shell command boundaries, normalizes backspace/delete bytes in line reads before copying command text to userspace, stages IRQ1-backed PS/2 keyboard input in a bounded broker-owned queue with set-1 scancode translation on BIOS boots, set-2 decoding support for framebuffer/UEFI handoff, and automatic set-1 fallback when high-bit set-1 release scancodes appear after a UEFI handoff, translates basic extended cursor/delete escape sequences, actively polls during authorized keyboard reads, clears stale pending bytes on scancode interpretation switches, discards overlong stale fragments before copying later lines, adds explicit keyboard byte-read and hardware-line-read syscalls that consume staged hardware bytes only after the caller presents scoped input authority, backs the generic byte-stream and line-read syscalls with the live keyboard queue instead of a deterministic seeded startup stream, starts the terminal queue empty at boot, leaves command execution to real typed input, and reports read/line/byte/edit/denial/eof plus keyboard IRQ/poll/scancode/translated-byte/pending/read/line/drop/last-key telemetry through int 0x80
...
input-keyboard: x64 brokered keyboard input now has a 256-byte pending queue and bounded multi-scancode PS/2 controller drains so live-shell verification has headroom without any compact scripted command stream while still reporting drops 0, QMP verification sends split press/release events with an 80 ms hold, 180 ms per-key pacing, 1300 ms post-line settling, and a 52000 ms probe window on disk media, while UEFI and ISO media use 210 ms per-key pacing, 1600 ms post-line settling, and a 56000 ms probe window so live command lines stay distinct without granting ambient input authority
...
optical-media: IMAPI2-packaged UEFI ISO generated from the packaged removable-media boot image; runtime boot proof is produced separately by tools\verify-qemu.ps1 -BootMedia iso
...
uefi-kernel-placement: UEFI app selects a 2 MiB-aligned address inside a conventional firmware region, allocates exact EfiLoaderData pages with AllocatePages, copies the verified kernel payload into that allocation, zeroes page padding, rechecks the copied bytes before reporting placement match 1, and then separately proves the linked x64 scaffold can be allocated and copied either at physical 0x10000 or through a dynamically selected conventional-memory fallback with entry 0xFFFFFFFF80010000 and the actual boot-info/page-root addresses recorded for the guarded kernel-entry handoff
...
uefi-boot-handoff: UEFI app first tries the legacy low handoff arena, then falls back to firmware-reported conventional memory when necessary; the selected arena builds the 16 MiB identity/high-half page-table substrate, framebuffer page-directory, kernel MMIO/APIC table pages, boot-info, and trampoline, and reports ready 1 with jump-ready 1 before leaving firmware
...
uefi-image: $uefiImageReport (packaged for OVMF QEMU BootMedia uefi verification; expected runtime proofs include GOP framebuffer, boot-info framebuffer mapping, kernel framebuffer draw, brokered display marker/panel/text proof, line-cleared scrolling brokered console-to-framebuffer mirror proof, brokered PS/2 plus xHCI HID keyboard event telemetry, explicit keyboard-read syscall proof, compact sealed bootstrap proof, real-media storage proofs, disk-sourced descriptor/binary launch proofs, boot-media file-read, loader buffer, dynamic kernel placement, dynamic boot-handoff, kernel-jump, and x64 userspace proofs)
uefi-stage: $uefiStageReport
uefi-iso: $isoReport (packaged for OVMF QEMU BootMedia iso verification; expected runtime proofs include GOP framebuffer, boot-info framebuffer mapping, kernel framebuffer draw, brokered display marker/panel/text proof, line-cleared scrolling brokered console-to-framebuffer mirror proof, brokered PS/2 plus xHCI HID keyboard event telemetry, explicit keyboard-read syscall proof, compact sealed bootstrap proof, real-media storage proofs, disk-sourced descriptor/binary launch proofs, boot-media file-read, loader buffer, dynamic kernel placement, dynamic boot-handoff, kernel-jump, and x64 userspace proofs)

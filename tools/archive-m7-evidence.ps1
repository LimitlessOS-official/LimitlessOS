...
$evidenceDir = Join-Path $distDir "m7-evidence-$timestamp"
...
function Update-M7InventoryEvidenceStatus
{
    ...
    $inventory | Add-Member -Force -NotePropertyName m7EvidenceDirectory -NotePropertyValue (Get-RepoRelativePath $EvidenceDirectory)
    try {
...
Copy-IfPresent -Path (Join-Path $root "docs\package-format.md") -Destination $artifactDir
...
$metadata = [ordered]@{
    milestone = "M7 Signed Package System"
    ...
    m8Blockers = @(
        "live public update fetching is not Product; M7 uses a signed local update-index fixture",

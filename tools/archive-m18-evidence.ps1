...
$evidenceDir = Join-Path $distDir "m18-evidence-$timestamp"
...
$metadata = [ordered]@{
    milestone = "M18 AI Consent-Scoped Action Mode"
...
$metadataPath = Join-Path $evidenceDir "m18-evidence.json"
$metadata | ConvertTo-Json -Depth 16 | Set-Content -LiteralPath $metadataPath -Encoding UTF8

$summaryPath = Join-Path $evidenceDir "m18-evidence-summary.md"
@(
    "# M18 Evidence Summary",
    "",
    "- Milestone: M18 AI Consent-Scoped Action Mode",
...
Write-Host "M18 evidence archived:"

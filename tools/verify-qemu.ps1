...
function Assert-X64M1RuntimeSurface
{
    ...
    if ($LoginExpected) {
        Assert-OutputContains -Lines $persistentLines -Pattern '^Product identity: Settings shows local account, association, vault, and trusted transport status; remote/cloud unavailable$' -Message "M13 runtime help did not describe Product identity/account/vault/transport status."
    }
    elseif ($BootMedia -ne "disk") {
        Assert-OutputContains -Lines $persistentLines -Pattern '^Product identity: Settings shows local account, association, vault, and trusted transport status; remote/cloud unavailable$' -Message "M13 UEFI runtime help did not describe identity/account/vault/transport visibility."
    }
    else {
    ...
    Assert-OutputContains -Lines $persistentLines -Pattern '^Unavailable in M13: ask \(not AI\), echo, aliases, personal-login, enterprise-login, account-linking, cloud-storage, encrypted-secrets, encrypted-identity-transport, credential-transport, token-storage, ai$' -Message "M13 runtime help did not label unavailable account/cloud/transport/AI surfaces."
...
elseif ($BootMedia -eq "disk") {
    ...
    Assert-OutputContains -Lines $outputLines -Pattern 'Unavailable in M13: ask \(not AI\), echo, aliases|Unavailable in M12: ask \(not AI\), echo, aliases|Unavailable in M11: ask \(not AI\), echo, aliases|Unavailable in M10: ask \(not AI\), echo, aliases|Unavail ASK-not-AI ECHO aliases' -Message "x64 ring-3 shell stream unavailable-surface help output was not observed."

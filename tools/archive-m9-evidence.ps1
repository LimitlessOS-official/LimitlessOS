...
$evidenceDir = Join-Path $distDir "m9-evidence-$timestamp"
...
function Get-InventoryPath
{
    ...
    return Join-Path $distDir ("limitlessos-x86_64.{0}.m9.json" -f $BuildProfile.ToLowerInvariant())
}
...
function Archive-ProfileArtifacts
{
    ...
    foreach ($milestone in @("m1", "m2", "m3", "m4", "m5", "m6", "m7", "m8", "m9")) {
        if ($milestone -eq "m1") {
...
function Update-M9InventoryEvidenceStatus
{
    ...
    $inventory | Add-Member -Force -NotePropertyName m9EvidenceDirectory -NotePropertyValue (Get-RepoRelativePath $evidenceDir)
    try {
...
Update-M9InventoryEvidenceStatus -GitStatusPath $gitStatusPath
...
$metadataPath = Join-Path $evidenceDir "m9-evidence.json"
$metadata | ConvertTo-Json -Depth 16 | Set-Content -LiteralPath $metadataPath -Encoding UTF8

$summaryPath = Join-Path $evidenceDir "m9-evidence-summary.md"
@(
    "# M9 Evidence Summary",
    "",
    "- Milestone: M9 Bare-Metal Validation + MSI Dry-Run Evidence",
...
Write-Host "M9 evidence archived:"

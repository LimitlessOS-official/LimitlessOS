...
$evidenceDir = Join-Path $distDir "m16-evidence-$timestamp"
...
function Get-InventoryPath
{
    ...
    return Join-Path $distDir ("limitlessos-x86_64.{0}.m16.json" -f $BuildProfile.ToLowerInvariant())
}
...
function Archive-ProfileArtifacts
{
    ...
    foreach ($milestone in @("m1", "m2", "m3", "m4", "m5", "m6", "m7", "m8", "m9", "m10", "m11", "m12", "m13", "m14", "m15", "m16")) {
        if ($milestone -eq "m1") {
...
$metadata = [ordered]@{
    milestone = "M16 AI Policy Broker Foundation"
    ...
    aiPolicyBrokerStatus = "Product request/deny/audit foundation; no AI actions"
    aiPrincipal = "system AI principal, no default capabilities, request-only"
    actionRequestModel = "request id, action, resource, capability, scope, reason/timestamp/origin modeled by Product telemetry"
    consentModel = "consent required; no auto-approval; M16 executes no action"
    auditLog = "immutable/queryable audit summary visible through Settings and pkginfo"
    deniedNoConsent = $true
    invalidScopeDenied = $true
    noAmbientAiAuthority = $true
    noFilesystemAccess = $true
    noNetworkAccess = $true
    noSettingsAccess = $true
    noPackageAccess = $true
    noSecretAccess = $true
    noCloudAccess = $true
    aiAssistantStatus = "unavailable"
    aiAutomationStatus = "unavailable"
    cloudAiStatus = "unavailable"
    noM17WorkStarted = $true
}

$metadataPath = Join-Path $evidenceDir "m16-evidence.json"
$metadata | ConvertTo-Json -Depth 16 | Set-Content -LiteralPath $metadataPath -Encoding UTF8

$summaryPath = Join-Path $evidenceDir "m16-evidence-summary.md"
@(
    "# M16 Evidence Summary",
    "",
    "- Milestone: M16 AI Policy Broker Foundation",
    ...
    "- AI policy broker: $($metadata.aiPolicyBrokerStatus)",
    "- AI principal: $($metadata.aiPrincipal)",
    "- Consent model: $($metadata.consentModel)",
    "- Audit log: $($metadata.auditLog)",
    "- Denial evidence: no consent, invalid scope, and no filesystem/network/settings/package/secret/cloud access",
    "- AI assistant/actions/automation/cloud AI: unavailable",
    "- Commands passed: $passedCount / $($records.Count)",
    "- No M17 work started."
) | Set-Content -LiteralPath $summaryPath -Encoding UTF8

Write-Host "M16 evidence archived:"

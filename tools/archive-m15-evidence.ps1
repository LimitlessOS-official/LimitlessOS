...
$evidenceDir = Join-Path $distDir "m15-evidence-$timestamp"
...
function Get-InventoryPath
{
    ...
    return Join-Path $distDir ("limitlessos-x86_64.{0}.m15.json" -f $BuildProfile.ToLowerInvariant())
}
...
function Archive-ProfileArtifacts
{
    ...
    foreach ($milestone in @("m1", "m2", "m3", "m4", "m5", "m6", "m7", "m8", "m9", "m10", "m11", "m12", "m13", "m14", "m15")) {
        if ($milestone -eq "m1") {
...
function Update-M15InventoryEvidenceStatus
{
    ...
    $inventory.installerWelcomeVerified = $true
    $inventory.installerBeginnerModeVerified = $true
    $inventory.installerAdvancedModeVerified = $true
    $inventory.installerHardwareSummaryVerified = $true
    $inventory.installerRecommendationVerified = $true
    $inventory.installerComponentSelectionVerified = $true
    $inventory.installerAccountPageVerified = $true
    $inventory.installerCloudPageVerified = $true
    $inventory.installerAiPageVerified = $true
    $inventory.installerPlanGenerated = $true
    $inventory.installerDryRunVerified = $true
    $inventory.installerWritesPlanned = 0
    $inventory.installerFormatsPlanned = 0
    $inventory.installerBootEntryChangesPlanned = 0
    $inventory.installerRealInstallApproved = $false
    $inventory.noAmbientInstallerAuthorityVerified = $true
    $inventory.noAmbientStorageAuthorityVerified = $true
    $inventory.noAmbientFirmwareAuthorityVerified = $true
    $inventory.noAmbientPackageAuthorityVerified = $true
    $inventory.noAmbientIdentityCloudSecretAuthorityVerified = $true
    $inventory.noM16WorkStarted = $true
    $inventory | Add-Member -Force -NotePropertyName m15EvidenceDirectory -NotePropertyValue (Get-RepoRelativePath $evidenceDir)
    try {
...
Update-M15InventoryEvidenceStatus -GitStatusPath $gitStatusPath
...
$metadata = [ordered]@{
    ...
    installerUxMode = "Product planning and dry-run only"
    welcomeScreen = "shows Product profile, boot safety, dry-run-safe status, write-disabled status, and mode buttons"
    beginnerMode = "read-only recommended general-use plan"
    advancedMode = "read-only disk/component/account/cloud/boot planning view"
    hardwareRecommendation = "deterministic general-use recommendation with unavailable-feature warnings"
    componentSelection = "Product components selected; unavailable components labeled planned/unavailable"
    accountSetupPage = "local account available; personal, enterprise, and security-key setup unavailable/planned"
    cloudStoragePage = "cloud broker status only; sync/upload/download/token storage unavailable"
    aiSetupPage = "AI-assisted setup unavailable/planned until consent broker exists"
    installPlan = "zero writes, zero formats, zero boot-entry changes, zero package installs, real install approval false"
    dryRunResult = "dry-run validation performs no writes and denies forbidden targets/actions"
    denialEvidence = "write, format, boot-entry, package install, cloud enable, AI enable, and ambient installer/storage/firmware/package/identity-cloud-secret authority denied"
    noM16WorkStarted = $true
}

$metadataPath = Join-Path $evidenceDir "m15-evidence.json"
$metadata | ConvertTo-Json -Depth 16 | Set-Content -LiteralPath $metadataPath -Encoding UTF8

$summaryPath = Join-Path $evidenceDir "m15-evidence-summary.md"
@(
    "# M15 Evidence Summary",
    "",
    "- Milestone: M15 Installer UX v2",
    ...
    "- Installer UX mode: $($metadata.installerUxMode)",
    "- Welcome screen: $($metadata.welcomeScreen)",
    "- Beginner mode: $($metadata.beginnerMode)",
    "- Advanced mode: $($metadata.advancedMode)",
    "- Hardware recommendation: $($metadata.hardwareRecommendation)",
    "- Component selection: $($metadata.componentSelection)",
    "- Account setup page: $($metadata.accountSetupPage)",
    "- Cloud storage page: $($metadata.cloudStoragePage)",
    "- AI setup page: $($metadata.aiSetupPage)",
    "- Install plan: $($metadata.installPlan)",
    "- Dry-run result: $($metadata.dryRunResult)",
    "- Denial evidence: $($metadata.denialEvidence)",
    "- Commands passed: $passedCount / $($records.Count)",
    "- No M16 work started."
) | Set-Content -LiteralPath $summaryPath -Encoding UTF8

Write-Host "M15 evidence archived:"

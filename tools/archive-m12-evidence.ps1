...
$evidenceDir = Join-Path $distDir "m12-evidence-$timestamp"
...
function Get-InventoryPath
{
    ...
    return Join-Path $distDir ("limitlessos-x86_64.{0}.m12.json" -f $BuildProfile.ToLowerInvariant())
}
...
function Archive-ProfileArtifacts
{
    ...
    foreach ($milestone in @("m1", "m2", "m3", "m4", "m5", "m6", "m7", "m8", "m9", "m10", "m11", "m12")) {
        if ($milestone -eq "m1") {
...
function Update-M12InventoryEvidenceStatus
{
    ...
    $inventory.identityTransportSettingsPanelVerified = $true
    $inventory.identityTransportStatusReadonlyVerified = $true
    $inventory.noAmbientIdTransportNetworkAuthorityVerified = $true
    $inventory.noAmbientIdTransportIdentityAuthorityVerified = $true
    $inventory.noAmbientIdTransportSecretAuthorityVerified = $true
    $inventory.noM13WorkStarted = $true
    $inventory | Add-Member -Force -NotePropertyName m12EvidenceDirectory -NotePropertyValue (Get-RepoRelativePath $evidenceDir)
    try {
...
Update-M12InventoryEvidenceStatus -GitStatusPath $gitStatusPath
...
$metadata = [ordered]@{
    milestone = "M12 Trusted Network Identity Transport"
    ...
    identityTransportMode = "Mode B endpoint trust foundation only"
    providerDescriptorFormat = "signed deterministic local fixture with provider id/type, endpoint key metadata, transport requirements, token policy, sequence, trusted-time metadata, signer key id, and Ed25519 signature"
    trustedEndpointVerification = "signed descriptor accepted; missing/invalid/wrong-key/tampered/rollback/unsupported-version descriptors denied"
    encryptedTransportStatus = "unavailable/non-product"
    credentialTransportStatus = "denied"
    tokenStorageStatus = "denied while vault is Mode B"
    trustedTimeStatus = "unavailable/non-product; expiry metadata not Product-enforced"
    personalAccountStatus = "planned/unavailable"
    enterpriseAccountStatus = "planned/unavailable"
    cloudAssociationStatus = "planned/unavailable"
    settingsIdentityTransportPanel = "read-only Product surface"
    shellStatusCommand = "pkginfo extended with read-only identity transport status; no mutation authority"
    denialEvidence = "descriptor negative fixtures, no-network-cap denial, plaintext credential denial, unverified endpoint denial, token storage denial, and no ambient identity/network/secret authority verified"
    m13WorkStarted = $false
}

$metadataPath = Join-Path $evidenceDir "m12-evidence.json"
$metadata | ConvertTo-Json -Depth 16 | Set-Content -LiteralPath $metadataPath -Encoding UTF8

$summaryPath = Join-Path $evidenceDir "m12-evidence-summary.md"
@(
    "# M12 Evidence Summary",
    "",
    "- Milestone: M12 Trusted Network Identity Transport",
    ...
    "- Identity transport mode: $($metadata.identityTransportMode)",
    "- Encrypted transport: $($metadata.encryptedTransportStatus)",
    "- Credential transport: $($metadata.credentialTransportStatus)",
    "- Token storage: $($metadata.tokenStorageStatus)",
    "- Trusted time: $($metadata.trustedTimeStatus)",
    "- Personal account: $($metadata.personalAccountStatus)",
    "- Enterprise account: $($metadata.enterpriseAccountStatus)",
    "- Cloud association: $($metadata.cloudAssociationStatus)",
    "- Settings panel: $($metadata.settingsIdentityTransportPanel)",
    "- Shell status command: $($metadata.shellStatusCommand)",
    "- Commands passed: $passedCount / $($records.Count)",
    "- No M13 work started."
) | Set-Content -LiteralPath $summaryPath -Encoding UTF8

Write-Host "M12 evidence archived:"

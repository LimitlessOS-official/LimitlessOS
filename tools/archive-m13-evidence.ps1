...
$evidenceDir = Join-Path $distDir "m13-evidence-$timestamp"
...
function Get-InventoryPath
{
    ...
    return Join-Path $distDir ("limitlessos-x86_64.{0}.m13.json" -f $BuildProfile.ToLowerInvariant())
}
...
function Archive-ProfileArtifacts
{
    ...
    foreach ($milestone in @("m1", "m2", "m3", "m4", "m5", "m6", "m7", "m8", "m9", "m10", "m11", "m12", "m13")) {
        if ($milestone -eq "m1") {
...
function Update-M13InventoryEvidenceStatus
{
    ...
    $inventory.accountSettingsPanelVerified = $true
    $inventory.accountShellStatusVerified = $true
    $inventory.accountMutationDenied = $true
    $inventory.accountUnlinkDenied = $true
    $inventory.cloudAssociationDenied = $true
    $inventory.tokenStorageDenied = $true
    $inventory.credentialTransportDenied = $true
    ...
    $inventory.noM14WorkStarted = $true
    $inventory | Add-Member -Force -NotePropertyName m13EvidenceDirectory -NotePropertyValue (Get-RepoRelativePath $evidenceDir)
    try {
...
Update-M13InventoryEvidenceStatus -GitStatusPath $gitStatusPath
...
$metadata = [ordered]@{
    milestone = "M13 Account Association"
    ...
    accountAssociationMode = "Mode B association policy/status only"
    localAssociationBehavior = "active/offline-capable; local login remains the only Product login"
    personalAssociationBehavior = "planned/unavailable"
    enterpriseAssociationBehavior = "planned/unavailable"
    cloudAssociationStatus = "planned/unavailable"
    securityKeyStatus = "planned/unavailable"
    tokenStorageStatus = "denied while vault is Mode B"
    encryptedTransportStatus = "unavailable/non-product"
    settingsAccountPanel = "read-only Product surface"
    shellAccountStatusCommand = "pkginfo extended with read-only account association status; no mutation authority"
    denialEvidence = "account mutation, unlink, credential transport, token storage, cloud association, enterprise policy, remote ambient authority, and ambient identity/network/secret authority denied"
    noM14WorkStarted = $true
}

$metadataPath = Join-Path $evidenceDir "m13-evidence.json"
$metadata | ConvertTo-Json -Depth 16 | Set-Content -LiteralPath $metadataPath -Encoding UTF8

$summaryPath = Join-Path $evidenceDir "m13-evidence-summary.md"
@(
    "# M13 Evidence Summary",
    "",
    "- Milestone: M13 Account Association",
    ...
    "- Account association mode: $($metadata.accountAssociationMode)",
    "- Local association: $($metadata.localAssociationBehavior)",
    "- Personal association: $($metadata.personalAssociationBehavior)",
    "- Enterprise association: $($metadata.enterpriseAssociationBehavior)",
    "- Cloud association: $($metadata.cloudAssociationStatus)",
    "- Security key: $($metadata.securityKeyStatus)",
    "- Token storage: $($metadata.tokenStorageStatus)",
    "- Encrypted transport: $($metadata.encryptedTransportStatus)",
    "- Settings account panel: $($metadata.settingsAccountPanel)",
    "- Shell account status command: $($metadata.shellAccountStatusCommand)",
    "- Commands passed: $passedCount / $($records.Count)",
    "- No M14 work started."
) | Set-Content -LiteralPath $summaryPath -Encoding UTF8

Write-Host "M13 evidence archived:"

...
$evidenceDir = Join-Path $distDir "m14-evidence-$timestamp"
...
function Get-InventoryPath
{
    ...
    return Join-Path $distDir ("limitlessos-x86_64.{0}.m14.json" -f $BuildProfile.ToLowerInvariant())
}
...
function Archive-ProfileArtifacts
{
    ...
    foreach ($milestone in @("m1", "m2", "m3", "m4", "m5", "m6", "m7", "m8", "m9", "m10", "m11", "m12", "m13", "m14")) {
        if ($milestone -eq "m1") {
...
function Update-M14InventoryEvidenceStatus
{
    ...
    $inventory.settingsCloudPanelVerified = $true
    $inventory.fileManagerCloudStatusVerified = $true
    $inventory.shellCloudStatusVerified = $true
    $inventory.cloudMutationDenied = $true
    $inventory.cloudUploadDenied = $true
    $inventory.cloudDownloadDenied = $true
    $inventory.cloudSyncDenied = $true
    $inventory.noAmbientCloudAuthorityVerified = $true
    $inventory.noAmbientFilesystemAuthorityVerified = $true
    $inventory.noAmbientNetworkAuthorityVerified = $true
    $inventory.noAmbientIdentityAuthorityVerified = $true
    $inventory.noAmbientSecretAuthorityVerified = $true
    $inventory.noM15WorkStarted = $true
    $inventory | Add-Member -Force -NotePropertyName m14EvidenceDirectory -NotePropertyValue (Get-RepoRelativePath $evidenceDir)
    try {
...
Update-M14InventoryEvidenceStatus -GitStatusPath $gitStatusPath
...
$metadata = [ordered]@{
    milestone = "M14 Cloud Storage Broker Foundation"
    ...
    cloudStorageBrokerMode = "Product foundation active; real cloud storage unavailable/planned"
    providerDescriptorBehavior = "signed deterministic local fixture verified; missing/invalid/wrong-key/tampered/rollback/version/malformed descriptors denied"
    cloudAccountStatus = "unavailable/planned"
    cloudAssociationStatus = "unavailable/planned"
    tokenStorageStatus = "denied while vault is Mode B"
    encryptedTransportStatus = "unavailable/non-product"
    uploadDownloadSyncStatus = "upload denied; download denied; sync unavailable"
    automaticUploadDownloadStatus = "unavailable"
    aiCloudAccessStatus = "unavailable"
    settingsCloudPanel = "read-only Product surface"
    fileManagerCloudStatus = "read-only unavailable-status area only; no cloud namespace is exposed"
    shellCloudStatusCommand = "pkginfo extended with read-only cloud status; no mutation authority"
    denialEvidence = "cloud upload/download/sync, Settings mutation, File Manager mutation, AI cloud access, app direct cloud authority, and ambient cloud/filesystem/network/identity/secret authority denied"
    noM15WorkStarted = $true
}

$metadataPath = Join-Path $evidenceDir "m14-evidence.json"
$metadata | ConvertTo-Json -Depth 16 | Set-Content -LiteralPath $metadataPath -Encoding UTF8

$summaryPath = Join-Path $evidenceDir "m14-evidence-summary.md"
@(
    "# M14 Evidence Summary",
    "",
    "- Milestone: M14 Cloud Storage Broker Foundation",
    ...
    "- Cloud storage broker mode: $($metadata.cloudStorageBrokerMode)",
    "- Provider descriptor: $($metadata.providerDescriptorBehavior)",
    "- Cloud account: $($metadata.cloudAccountStatus)",
    "- Cloud association: $($metadata.cloudAssociationStatus)",
    "- Token storage: $($metadata.tokenStorageStatus)",
    "- Encrypted transport: $($metadata.encryptedTransportStatus)",
    "- Upload/download/sync: $($metadata.uploadDownloadSyncStatus)",
    "- Automatic upload/download: $($metadata.automaticUploadDownloadStatus)",
    "- AI cloud access: $($metadata.aiCloudAccessStatus)",
    "- Settings cloud panel: $($metadata.settingsCloudPanel)",
    "- File Manager cloud status: $($metadata.fileManagerCloudStatus)",
    "- Shell cloud status command: $($metadata.shellCloudStatusCommand)",
    "- Commands passed: $passedCount / $($records.Count)",
    "- No M15 work started."
) | Set-Content -LiteralPath $summaryPath -Encoding UTF8

Write-Host "M14 evidence archived:"

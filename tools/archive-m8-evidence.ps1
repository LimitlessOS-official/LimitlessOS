...
$evidenceDir = Join-Path $distDir "m8-evidence-$timestamp"
...
function Get-InventoryPath
{
    ...
    return Join-Path $distDir ("limitlessos-x86_64.{0}.m8.json" -f $BuildProfile.ToLowerInvariant())
}
...
function Archive-ProfileArtifacts
{
    ...
    foreach ($milestone in @("m1", "m2", "m3", "m4", "m5", "m6", "m7", "m8")) {
        if ($milestone -eq "m1") {
...
function Export-PackageTrustOutput
{
    $statusPath = Join-Path $evidenceDir "package-trust-status.txt"
    $lines = New-Object System.Collections.Generic.List[string]
    foreach ($file in Get-ChildItem -LiteralPath $commandDir -Filter "*.output.txt") {
        foreach ($line in Get-Content -LiteralPath $file.FullName) {
            if (($line -match 'drs-pkg') -or ($line -match 'package system:') -or ($line -match 'trusted public key') -or ($line -match 'auto-install:') -or ($line -match 'public update fetch:') -or ($line -match 'trusted-time expiry:')) {
...
function Update-M8InventoryEvidenceStatus
{
    ...
    $inventory.packageTrustSurfaceVerified = $true
    $inventory.settingsPackagePanelVerified = $true
    $inventory.shellPackageStatusVerified = $true
    $inventory | Add-Member -Force -NotePropertyName m8EvidenceDirectory -NotePropertyValue (Get-RepoRelativePath $evidenceDir)
    try {
...
$packageTrustPath = Export-PackageTrustOutput
Update-M8InventoryEvidenceStatus -GitStatusPath $gitStatusPath
...
$metadata = [ordered]@{
    milestone = "M8 Package Manager UX + Trust Policy Surface"
    ...
    packageTrustStatusOutput = Get-RepoRelativePath $packageTrustPath
    ...
    packageTrustSurfaceVerified = if ($productInventory) { $productInventory.packageTrustSurfaceVerified } else { $null }
    settingsPackagePanelVerified = if ($productInventory) { $productInventory.settingsPackagePanelVerified } else { $null }
    shellPackageStatusVerified = if ($productInventory) { $productInventory.shellPackageStatusVerified } else { $null }
    packageManagerUxStatus = if ($productInventory) { $productInventory.packageManagerUxStatus } else { $null }
    autoInstallStatus = if ($productInventory) { $productInventory.autoInstallStatus } else { $null }
    publicUpdateFetchStatus = if ($productInventory) { $productInventory.publicUpdateFetchStatus } else { $null }
    trustedTimeStatus = if ($productInventory) { $productInventory.trustedTimeStatus } else { $null }
    installActionStatus = if ($productInventory) { $productInventory.installActionStatus } else { $null }
    updateApplyActionStatus = if ($productInventory) { $productInventory.updateApplyActionStatus } else { $null }
    biosPackageMode = if ($productInventory) { $productInventory.biosPackageMode } else { $null }
    uefiPackageMode = if ($productInventory) { $productInventory.uefiPackageMode } else { $null }
    signerKeyId = if ($productInventory) { $productInventory.signerKeyId } else { $null }
    signerFingerprint = if ($productInventory) { $productInventory.signerFingerprint } else { $null }
    ...
    m9Blockers = @(
        "no app store or package install/apply UX yet",
...
$metadataPath = Join-Path $evidenceDir "m8-evidence.json"
$metadata | ConvertTo-Json -Depth 12 | Set-Content -LiteralPath $metadataPath -Encoding UTF8

$summaryPath = Join-Path $evidenceDir "m8-evidence-summary.md"
@(
    "# M8 Evidence Summary",
    "",
    "- Milestone: M8 Package Manager UX + Trust Policy Surface",
    ...
    "- Settings package panel verified: $($metadata.settingsPackagePanelVerified)",
    "- Shell package status verified: $($metadata.shellPackageStatusVerified)",
    "- Auto-install status: $($metadata.autoInstallStatus)",
    "- Public update fetch status: $($metadata.publicUpdateFetchStatus)",
    "- Trusted-time status: $($metadata.trustedTimeStatus)",
    "- M7.1 negative fixtures preserved: $($metadata.m7_1NegativeFixturesComplete)",
    "- Commands passed: $((@($records.ToArray()) | Where-Object { $_.exitCode -eq 0 }).Count) / $($records.Count)",
    "- No M9 work started."
) | Set-Content -LiteralPath $summaryPath -Encoding UTF8

Write-Host "M8 evidence archived:"
